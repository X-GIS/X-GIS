...
export function getStyleProfile(scene: Scene): StyleProfile {
  ...
  const redundancy = totalCSE > 0 ? Math.round(((totalCSE - uniqueCSE) / totalCSE) * 1000) / 10 : 0
...
export function formatStyleProfile(p: StyleProfile): string {
  ...
  lines.push('')
  lines.push(
    `CSE: ${p.cse.unique}/${p.cse.totalNodes} unique nodes ` +
