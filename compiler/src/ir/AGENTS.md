...
| `lower-label-fold.ts`                                                                                      | Pure assembly half of `lower-label.ts`: `foldLabelKnobs` merges the accumulated `label-*` knob values onto the `LabelDef` built from `label-[<expr>]` and builds its `LabelShapes` bundle. One call site, no shared state. A NEW label knob touches both halves — accumulator + loop arm in `lower-label.ts`, `knobs` field + merged spread here.                         |
