...
//
// The pure assembly half (`foldLabelKnobs`) lives in `lower-label-fold.ts`
// (#2051 §9): a new knob adds its accumulator + loop arm HERE and its `knobs`
// field + merged spread THERE.
...
import { type ZoomStop, type Diagnostic, hexToRgba } from './render-node'
import { foldLabelKnobs } from './lower-label-fold'
...
export function lowerLabelProps(
  ...
  /** Mapbox `text-writing-mode` (#2051 / design doc §12 P1) — only the
   *  non-default `vertical` is carried; `horizontal` is the spec default and
   *  emits no utility at all, so it stays undefined here. */
  let labelWritingMode: 'vertical' | undefined
  ...
  for (const line of expandedUtilities) {
    for (const item of line.items) {
      ...
      // #2051 — the converter emits ONLY the non-default orientation, so this
      // is an exact match rather than a `label-writing-mode-` prefix arm: a
      // future `-horizontal` (or a typo) then reaches the X-GIS0006 catch-all
      // instead of being silently accepted as "not vertical".
      if (name === 'label-writing-mode-vertical') {
