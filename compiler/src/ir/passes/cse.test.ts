...
const fnCall = (
  calleeName: string,
  args: unknown[],
  matchArms: Array<{ pattern: string; value: unknown }> = [],
) => ({
...
describe('analyzeCSE — basic walk', () => {
  ...
  it('shared `get(.class)` across fill + stroke → 1 duplicate (count 2)', () => {
    ...
    // surface it.
    const fillExpr = fnCall(
      'match',
      [field('class')],
      [{ pattern: 'a', value: { kind: 'ColorLiteral', value: '#aaa' } }],
    )
    const strokeExpr = fnCall(
      'match',
      [field('class')],
    ...
    const report = analyzeCSE(
      makeScene(
        makeNode({
          fill: {
            kind: 'conditional',
            branches: [{ field: 'urban', value: { kind: 'data-driven', expr: expr(inner) } }],
