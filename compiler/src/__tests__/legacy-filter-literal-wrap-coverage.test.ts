...
describe('legacy filter forms with literal-wrapped values', () => {
  it('["==", "kind", ["literal", "park"]] → .kind == "park"', () => {
    const w: string[] = []
    expect(filterToXgis(['==', 'kind', ['literal', 'park']], w)).toBe('.kind == "park"')
  })

  it('["!=", "kind", ["literal", "water"]] → .kind != "water"', () => {
    const w: string[] = []
    expect(filterToXgis(['!=', 'kind', ['literal', 'water']], w)).toBe('.kind != "water"')
  })

  it('["!in", "kind", ["literal", "park"], "forest"] mixed wrap form', () => {
    const w: string[] = []
    expect(filterToXgis(['!in', 'kind', ['literal', 'park'], 'forest'], w)).toBe(
  ...
  it('[">=", "level", ["literal", 3]] → .level >= 3 (numeric wrap)', () => {
    const w: string[] = []
    expect(filterToXgis(['>=', 'level', ['literal', 3]], w)).toBe('.level >= 3')
  })
  ...
  it('bare legacy form still works (regression guard)', () => {
    ...
    expect(filterToXgis(['!in', 'kind', 'park', 'forest'], w)).toBe(
      '.kind != "park" && .kind != "forest"',
    )
  })
})
