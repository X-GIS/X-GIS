...
describe('icon-rotation-alignment=map — compile-time propagation', () => {
  it('"map" + symbol-placement=line → label.iconRotationAlignment="map"', () => {
    const shows = compileToShows(symbol({
      'icon-image': 'oneway',
      'icon-rotation-alignment': 'map',
      'symbol-placement': 'line',
    }))
    expect((shows[0]!.label as { iconRotationAlignment?: string }).iconRotationAlignment).toBe('map')
  })
  ...
  it('"viewport" → iconRotationAlignment undefined (matches X-GIS default render)', () => {
    ...
    expect((shows[0]!.label as { iconRotationAlignment?: string }).iconRotationAlignment).toBeUndefined()
  })
  ...
  it('OFM road_oneway shape: icon-rotate=90 + rot-align=map + placement=line', () => {
    ...
    const style = {
      ...
      sources: { o: { type: 'vector', tiles: ['http://x/{z}/{x}/{y}.pbf'] } },
      layers: [{
        id: 'road_oneway', type: 'symbol', source: 'o', 'source-layer': 'transportation',
        minzoom: 15,
        layout: {
          'icon-image': 'oneway',
          'icon-rotate': 90,
  ...
  it('"map" does NOT trigger the ignored-property warning anymore (iter 506)', () => {
    ...
    const xgis = convertMapboxStyle(symbol({
      'icon-image': 'arrow',
      'icon-rotation-alignment': 'map',
      'symbol-placement': 'line',
