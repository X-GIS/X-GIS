...
describe('distance converter', () => {
  ...
  it('LineString target → segment pairs', () => {
    const { result } = convert(['distance', { type: 'LineString', coordinates: [[0, 0], [0, 1], [0, 2]] }])
    expect(result).toBe('distance(get("$geometry"), [], [[[0, 0], [0, 1]], [[0, 1], [0, 2]]], [])')
  })

  it('Polygon target → ring-set in polygons AND edges in segments', () => {
    const { result } = convert(['distance', { type: 'Polygon', coordinates: [[[0, 0], [1, 0], [1, 1], [0, 0]]] }])
    expect(result).toBe(
  ...
  it('end-to-end: distance to a point evaluates to ~110 km', () => {
    ...
    const d = evalSrc(result as string, { $geometry: { type: 'Point', coordinates: [0, 0] } }) as number
