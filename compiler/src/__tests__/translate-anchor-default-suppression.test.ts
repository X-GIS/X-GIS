...
const CASES: Case[] = [
  { layer: 'fill',           property: 'fill-translate-anchor',           parent: 'fill-translate',   mapSupported: true },
  { layer: 'line',           property: 'line-translate-anchor',           parent: 'line-translate',   mapSupported: true },
  { layer: 'circle',         property: 'circle-translate-anchor',         parent: 'circle-translate', mapSupported: false },
...
describe('*-translate-anchor default suppression', () => {
  for (const c of CASES) {
    ...
    it(`${c.property} 'map' WITH parent → ${c.mapSupported ? 'no warning (supported)' : 'warns (real gap)'}`, () => {
      ...
      const w = coverage.warnings.find(w => w.includes(c.property))
      if (c.mapSupported) {
        expect(w, `${c.property}=map is now supported and should not warn: ${JSON.stringify(coverage.warnings)}`).toBeUndefined()
      } else {
