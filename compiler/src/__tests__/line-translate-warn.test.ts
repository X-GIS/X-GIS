...
describe('line-translate — Mapbox → ShowCommand.strokeTranslateX/Y', () => {
  ...
  it('[0, 0] → both undefined (default, no-op)', () => {
    const shows = compileToShows(lineLayer('zero', {
      'line-color': '#888', 'line-translate': [0, 0],
    }))
    expect(shows[0]!.strokeTranslateX).toBeUndefined()
    expect(shows[0]!.strokeTranslateY).toBeUndefined()
  })
  ...
  it('[2, 3] → x=2, y=3', () => {
    const shows = compileToShows(lineLayer('pos', {
      'line-color': '#888', 'line-translate': [2, 3],
    }))
  ...
  it('[-2, -3] → x=-2, y=-3 via bracket form', () => {
    const shows = compileToShows(lineLayer('neg', {
      'line-color': '#888', 'line-translate': [-2, -3],
    }))
    expect(shows[0]!.strokeTranslateX).toBe(-2)
    expect(shows[0]!.strokeTranslateY).toBe(-3)
  ...
  it('only x non-zero: [4, 0] → x=4, y undefined', () => {
    const shows = compileToShows(lineLayer('xOnly', {
      'line-color': '#888', 'line-translate': [4, 0],
    }))
  ...
  it('only y non-zero: [0, -5] → x undefined, y=-5', () => {
    const shows = compileToShows(lineLayer('yOnly', {
      'line-color': '#888', 'line-translate': [0, -5],
    }))
    expect(shows[0]!.strokeTranslateX).toBeUndefined()
  ...
  it('v8 ["literal", [dx, dy]] wrap unwraps to constant', () => {
    const shows = compileToShows(lineLayer('v8wrap', {
      'line-color': '#888', 'line-translate': ['literal', [-1, 3]],
    }))
  ...
  it('null → both undefined (spec fallback, no warning)', () => {
    ...
    convertMapboxStyle(lineLayer('nullt', { 'line-color': '#888', 'line-translate': null }) as never, { coverage })
