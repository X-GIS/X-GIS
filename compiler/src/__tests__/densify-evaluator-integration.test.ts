...
import { convertMapboxStyle, Lexer, Parser, lower, emitCommands, evaluate, CAMERA_ZOOM_KEY } from '../index'
...
describe('cubic-bezier densification integration — compile → evaluator', () => {
      ...
      layers: [{
        ...
        paint: {
          ...
          'line-width': ['interpolate',
            ['cubic-bezier', 0.42, 0, 0.58, 1], ['zoom'],
            10, 0, 20, 100,
          ],
        },
      }],
    ...
    const style = {
      ...
      layers: [{
        ...
        paint: {
          'line-color': '#fff',
          'line-width': ['interpolate',
            ['cubic-bezier', 0.25, 0.1, 0.25, 1], ['zoom'],
            10, 3, 20, 17,
          ],
        },
      }],
  ...
  it('cssBezierEase agrees with the densified evaluator output at sample points', () => {
    ...
    const x1 = 0.65, y1 = 0, x2 = 0.35, y2 = 1 // ease-in-out-back-ish
    const style = {
      version: 8,
      sources: { v: { type: 'vector', tiles: ['https://example.com/{z}/{x}/{y}.pbf'] } },
      layers: [{
    ...
    for (const z of [1, 2.5, 5, 7.5, 9]) {
      ...
      expect(Math.abs(runtime - expected), `z=${z}: runtime=${runtime}, expected=${expected}`).toBeLessThan(0.05)
    }
  })
})
