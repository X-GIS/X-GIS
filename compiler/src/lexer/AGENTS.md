...
Contains the tokenizer for the `.xgis` style language. `lexer.ts` scans raw source text into a flat `Token[]` array that the parser consumes. `tokens.ts` defines the complete token vocabulary: literals (Number, String, Color `#hex`, Bool), all map-domain keywords (`source`, `layer`, `background`, `keyframes`, `symbol`, `preset`, …), seven physical-unit suffixes (`px`, `m`, `km`, `nm`, `deg`, `s`, `ms`), and the full operator/punctuation set including `->`, `??`, `..`, and logical operators. This is stage 1 of the compiler pipeline: lexer → parser → IR + passes → codegen → binary.
...
- Color validation enforces exactly 3, 4, 6, or 8 hex digits after `#` (`#RGB`, `#RGBA`, `#RRGGBB`, `#RRGGBBAA`) — the 4-digit CSS Color Module 4 shorthand was added in #2543 to match the Mapbox converter and every downstream colour resolver. This is a lex-time error, not a parse-time error.
