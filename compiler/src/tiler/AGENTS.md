...
| File | Description |
|------|-------------|
| `vector-tiler.ts` | Core tiler (~2015 LOC). `compileGeoJSONToTiles(Async)`, `compileSingleTile`, `decomposeFeatures`, DSFUN/ECEF vertex packing (`packECEFPolygonVertices`, `packECEFLineSegments`, `packECEFPointFeatures`, `packDSFUNLineVertices`), per-tile tile math, great-circle subdivision, earcut tessellation with triangle densification for non-Mercator projections, backtrack-repair probe. Emits `CompiledTileSet`/`CompiledTile`. Re-exports tile-key helpers and types from the split files. |
