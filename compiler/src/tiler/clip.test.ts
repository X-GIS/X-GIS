...
describe('clipPolygonToRectV2 — parity with V1 on the same cases', () => {
  ...
  const cases: { name: string; ring: number[][]; expectLen: number }[] = [
    {
      name: 'big square containing rect',
      ring: [
        [-10, -10],
        [10, -10],
        [10, 10],
    ...
    {
      name: 'elongated ribbon containing rect',
      ring: [
        [-180, -5],
        [180, -5],
        [180, 5],
    ...
    {
      name: 'one corner pokes into rect',
      ring: [
        [0.5, 0.5],
        [5, 0.5],
        [5, 5],
    ...
    {
      name: 'CW-wound ring containing rect',
      ring: [
        [-10, -10],
        [-10, 10],
        [10, 10],
    ...
    {
      name: 'unclosed ring containing rect',
      ring: [
        [-10, -10],
        [10, -10],
        [10, 10],
    ...
    {
      name: 'narrow U-shape with bottom under rect',
      ring: [
        [-10, -2],
        [-0.5, -2],
        [-0.5, 5],
...
describe('clipPolygonToRect — Yellow Sea repro coordinates', () => {
  ...
  it('Mercator-scale ring containing a z=7 sub-tile → produces tile rect', () => {
    ...
    expect(out, 'world-wrapping ocean ring should produce a rect for tiles inside it').toHaveLength(
      1,
    )
    if (out.length > 0) {
      expect(out[0].length).toBeGreaterThanOrEqual(4)
