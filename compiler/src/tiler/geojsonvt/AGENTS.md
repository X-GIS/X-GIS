...
| File | Description |
|------|-------------|
| `index.ts` | Public entry: `geojsonvt`, `GeoJSONVT`, `DEFAULT_OPTIONS`. The in-memory index — DP simplification + per-axis range clipping + recursive `splitTile`, integrated with X-GIS `tileKey`. |
...
| `encode-mvt.ts` | X-GIS addition: `encodeMVT` — tiled features → MVT v2.1 PBF bytes (mirrors vt-pbf output). `MVTLayerInput`/`EncodeOptions`. |
...
- The single intentional divergence from upstream is tile addressing (Morton `tileKey` from `../vector-tiler.ts`, accurate to z=25, vs geojson-vt's `1<<z` 32-bit pack that wraps at z=31). Keep that integration documented at the call site.
- `encode-mvt.ts` is the X-GIS-original file here; it's where MVT-spec output behavior belongs.
...
### Internal
- `index.ts` uses `../vector-tiler.ts` (`tileKey`); re-exported via `src/index.ts`.

### External
- None at runtime (the port replaces `geojson-vt`/`vt-pbf`, which remain dev-only references).
