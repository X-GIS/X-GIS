...
const PAINT_LINE: readonly CoverageEntry[] = [
  ...
  { name: 'line-translate', status: 'partial', impact: 'low', note: 'Constant vec2 + zoom-interp last-stop approx end-to-end (mirrors fill-translate). Converter emits stroke-translate-x-N / stroke-translate-y-M utilities; lower.ts parses them into strokeTranslateX/Y on the render node; VTR bakes CSS px → NDC-per-pixel and writes into LineLayer uniform slots 48/49 (u.line_translate_x/y). Applied in vs_line post-MVP (`clip.xy += u.line_translate * clip.w`). viewport anchor only — map-space translate deferred.', source: 'paint.ts:addLineTranslate' },
...
const PAINT_CIRCLE: readonly CoverageEntry[] = [
  ...
  { name: 'circle-translate',    status: 'partial', impact: 'low', note: 'Constant [dx, dy] vec2 + zoom-interp last-stop approximation supported. Emits circle-translate-x-N / circle-translate-y-M utilities; lower.ts threads to ShowCommand.circleTranslateX/Y; point uniform circle_params.xy baked to NDC-per-pixel by PointRenderer. Full per-frame zoom-interp deferred (same constraint as fill-translate).', source: 'layers-circle.ts:circle-translate block' },
