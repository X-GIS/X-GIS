...
const PAINT_LINE: readonly CoverageEntry[] = [
  ...
  { name: 'line-dasharray', status: 'partial', impact: 'medium', note: 'Constant numeric array only — interpolate-by-zoom dasharray not lowered. Iter 27 sharpened the non-constant warning to name the specific shape (zoom-interp needs PropertyShape<array>; data-driven needs per-feature dash plumbing).', source: 'paint.ts:126' },
  ...
  { name: 'line-gradient',  status: 'unsupported', impact: 'low', note: 'Gradient along the line via ["line-progress"]. Full dependency chain (iter-158 scoping, the renderer change is NOT the hard part): (1) PREREQUISITE — geojson-vt currently IGNORES source.lineMetrics (geojsonvt/index.ts:14, sources.ts:406). line-progress is normalised over the ORIGINAL feature but geojson-vt clips lines per tile, so the clip stage must track each clipped segment\'s [progressStart,progressEnd] fraction of the original arc-length. This compiler-tiler change is the bulk of the work. (2) line-segment-build.ts interpolates per-vertex progress 0..1. (3) new per-vertex progress attribute + WGSL line fragment samples a gradient LUT the converter emits from the line-gradient interpolate stops. ~5 files; multi-day; not a surgical fix. Iter 22 added the gap warning citing only the accessor — scope understated until now.' },
]
...
const PAINT_SYMBOL: readonly CoverageEntry[] = [
  ...
  { name: 'icon-halo-color',  status: 'unsupported', impact: 'medium', note: 'SDF icon halo colour. The SDF icon fragment path now EXISTS (iter 138 icon-color, icon-renderer.ts fs sdf branch: edge 0.5 + fwidth AA + per-vertex tint). Remaining blocker (iter-159 sharpened): the shader part is trivial (second smoothstep at edge-haloWidth, mirror fs_text). The IRREDUCIBLE blocker is the halo-width→SDF-distance normalisation: sprites carry an EXTERNALLY pre-baked SDF in the PNG alpha (spritezero, server-side; X-GIS has no sprite-SDF generator), so the spritezero buffer constant is NOT in the codebase and is NOT derivable — guessing it violates project_military_precision (no casual approximation on real-world data; iter-138 only empirically fit edge=0.5 for FILL, which tolerates slop; halo width does not). FIRST STEP must be a MapLibre icon-halo pixel-diff probe to pin the buffer constant from real OFM sprite data (same gate #1 needs). NOT a code-only fix.' },
  { name: 'icon-halo-width',  status: 'unsupported', impact: 'medium', note: 'SDF icon halo width. Blocked only on the sprite-SDF-buffer halo-width normalisation noted under icon-halo-color (the SDF icon path itself shipped iter 138).' },
  { name: 'icon-halo-blur',   status: 'unsupported', impact: 'low', note: 'SDF icon halo feather. Same narrow blocker as icon-halo-width (sprite SDF spread normalisation on the iter-138 SDF icon path).' },
  { name: 'icon-translate',   status: 'unsupported', impact: 'low', note: 'CSS-px viewport offset for icons. Symmetric with line-translate / fill-translate; not threaded through IconStage. Iter 35 added a specific gap warning noting X-GIS shares text-translate offset for both icon and text today.' },
...
const PAINT_CIRCLE: readonly CoverageEntry[] = [
  ...
  { name: 'circle-stroke-opacity', status: 'partial', impact: 'low', note: 'Constant numeric form folds into stroke-color hex alpha (iter 4, Plan §4 partial landing — same pattern later applied to background-opacity in iter 47). Zoom-interp / data-driven forms still warn + drop — need a dedicated paint shape for per-frame uniform multiplication.', source: 'layers.ts:circle-stroke-color block' },
  { name: 'circle-translate',    status: 'unsupported', impact: 'low', note: 'CSS-px viewport offset for circles. Symmetric with fill/line/icon translate. Iter 34 added a specific gap warning naming the missing point-renderer translate uniform.' },
...
const EXPRESSIONS: readonly CoverageEntry[] = [
  ...
  { name: 'interpolate-hcl',           status: 'partial', impact: 'low', note: 'Zoom AND data-driven interpolates over hex colour stops densify in LCh (polar Lab) space at compile time (iter 61-62, 6 samples per segment, hue shortest-path). Runtime sees a longer linear-sRGB hex stop list and visually approximates the LCh-space interpolation. Non-hex stops fall back to linear-sRGB with a graceful-downgrade warning.', source: 'paint.ts + expressions.ts:interpolate handler' },
  { name: 'interpolate-lab',           status: 'partial', impact: 'low', note: 'Zoom AND data-driven interpolates over hex colour stops densify in Lab (D50) space at compile time (iter 61-62, 6 samples per segment). Runtime sees a longer linear-sRGB hex stop list and visually approximates the Lab-space interpolation. Non-hex stops fall back to linear-sRGB with a graceful-downgrade warning.', source: 'paint.ts + expressions.ts:interpolate handler' },
  // Feature meta
