...
const PAINT_FILL: readonly CoverageEntry[] = [
  ...
  { name: 'fill-pattern',       status: 'unsupported', impact: 'high', note: 'Bitmap pattern repeated across polygon. iter-165 probe of OFM bright/liberty: bright uses 0 patterns; liberty uses 2 (`pedestrian_polygon` 64×64 + `wetland_bg_11` 15×15, both raster in the OFM atlas — confirmed via playground/scripts/sprite-sdf-buffer-probe). impact stays high — real visual gap for liberty pedestrian areas + wetlands. Implementation = multi-file unit (compiler emit `fill-pattern-<name>` utility + IR ShowCommand field + runtime NEW fill pipeline variant with sprite atlas texture + sampler + UV-tiling fragment shader). NOT a single-iteration item; compiler-only slice would be dead code (CLAUDE.md §2). Reuses iter-138 sprite atlas infra.', source: 'paint.ts:166 specific warning' },
...
const PAINT_LINE: readonly CoverageEntry[] = [
  ...
  { name: 'line-pattern',   status: 'unsupported', impact: 'low', note: 'Bitmap pattern repeated along line. iter-165 probe: ZERO uses in OFM bright/liberty target fixtures — impact reclassified medium → low (no visible effect on dominant styles). Implementation = line-renderer pattern pipeline variant + along-line UV (similar to line-gradient line-progress prerequisite, separate effort). Layer falls back to solid line-color if pattern is the only visual cue.' },
...
const PAINT_FILL_EXTRUSION: readonly CoverageEntry[] = [
  ...
  { name: 'fill-extrusion-pattern',   status: 'unsupported', impact: 'low', note: 'Bitmap pattern repeated on extrusion walls + roof. iter-165 probe: ZERO uses in OFM bright/liberty target fixtures — impact reclassified medium → low. Same UV-tiling fragment dependency as fill-pattern (would reuse that pipeline variant once shipped).' },
