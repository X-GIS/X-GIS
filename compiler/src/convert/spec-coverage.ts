...
const PAINT_SYMBOL: readonly CoverageEntry[] = [
  ...
  { name: 'icon-halo-color',  status: 'unsupported', impact: 'medium', note: 'SDF icon halo colour. The SDF icon fragment path now EXISTS (iter 138 icon-color, icon-renderer.ts fs sdf branch: edge 0.5 + fwidth AA + per-vertex tint). Remaining blocker is narrow: add a second smoothstep at edge-haloWidth (mirror text-renderer fs_text halo composite) + the halo-width normalisation against the sprite SDF buffer/radius (the genuine open unknown — sprite SDF spread metadata, unlike glyphs where haloK=24/8 is fixed). Not "needs SDF support first" — that landed.' },
