...
const PAINT_LINE: readonly CoverageEntry[] = [
  ...
  { name: 'line-pattern',   status: 'partial', impact: 'low', note: 'iter-178 Stage 1 landed 2026-05-20: compiler emits `stroke-pattern-<name>` utility; lower.ts threads it into ShowCommand.linePattern; map.ts samples the sprite\'s centre pixel and stores it as `resolvedStrokeRgba`. Lines whose only stroke declaration was `line-pattern: X` now render in the sprite\'s intended colour band instead of staying invisible. Stage 2 (true repeating-sprite stroke renderer with along-line UV) still pending — visual is a flat colour, not the repeating bitmap pattern MapLibre renders. Constant string form only.', source: 'paint.ts:210 iter-178' },
...
const PAINT_FILL_EXTRUSION: readonly CoverageEntry[] = [
  ...
  { name: 'fill-extrusion-pattern',   status: 'partial', impact: 'low', note: 'iter-179 Stage 1 landed 2026-05-20: compiler routes fill-extrusion-pattern through the same `fill-pattern-<name>` utility as iter-177, since the extrude renderer multiplies the layer fill RGBA by wall_shade in the fragment shader. Pattern-only building styles now render walls in the sprite\'s centre-pixel colour band instead of uncoloured. Stage 2 (real UV-tiling fragment shader on walls + roof) still pending. Constant string form only.', source: 'paint.ts:270 iter-179' },
