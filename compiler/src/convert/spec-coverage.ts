...
const EXPRESSIONS: readonly CoverageEntry[] = [
  ...
  { name: 'interpolate-hcl',           status: 'partial', impact: 'low', note: 'Zoom interpolates over hex colour stops densify in LCh (polar Lab) space at compile time (iter 61, 6 samples per segment, hue shortest-path). Runtime sees a longer linear-sRGB hex stop list and visually approximates the LCh-space interpolation. Non-zoom inputs + non-hex stops fall back to linear-sRGB with a graceful-downgrade warning.', source: 'paint.ts:interpolateZoomStops' },
  { name: 'interpolate-lab',           status: 'partial', impact: 'low', note: 'Zoom interpolates over hex colour stops densify in Lab (D50) space at compile time (iter 61, 6 samples per segment). Runtime sees a longer linear-sRGB hex stop list and visually approximates the Lab-space interpolation. Non-zoom inputs + non-hex stops fall back to linear-sRGB with a graceful-downgrade warning.', source: 'paint.ts:interpolateZoomStops' },
  // Feature meta
