...
export const LAYOUT_SYMBOL: readonly CoverageEntry[] = [
  ...
  },
  {
    ...
    status: 'partial',
    impact: 'medium',
    note: "The spec DEFAULT (`false`) was never a gap — a collision-rejected label stamps its pairKey into TextStage.droppedPairKeys and IconStage skips the paired icon, which IS MapLibre's text+icon-as-one-symbol rule. #2440 closed the `true` case for POINT-placed symbols: converter emits `label-text-optional` -> LabelDef.textOptional -> one predicate in text-stage.ts that both suppresses the cascade stamp AND removes the pairKey from getActiveTextPairKeys(). The second half is the load-bearing one and the reason this is not 'just stop stamping': #609 skips a paired icon's obstacle box on the premise that the icon is either represented by its live text's bbox or dropped with it, and `text-optional: true` falsifies the second branch — without the removal the surviving icon would draw while participating in no collision at all. The earlier note said this needed SPLIT text/icon arbitration; it does not. The over-block cost of always seeding that box is zero when the text wins, structurally rather than by measurement: PairedBadgeBoxes.union (paired-symbol-box.ts:105-119) has already grown the live text's bbox to the icon's half-extents on the shared anchor, computed by pairedIconHalfExtents with the same arithmetic computeObstacles uses — the identical rectangle at the default `icon-anchor: center`. PARTIAL, not supported, for three named residuals: (1) a NON-CENTER icon-anchor, where computeObstacles shifts its box while union centres it, so a sliver bounded by the anchor offset can over-block — a pre-existing asymmetry between the two paths, not introduced here; (2) CURVED-LINE paired labels, whose union centres on the walked box rather than the icon anchor, so the zero-cost argument does not extend there (the fixture that motivated this — the OFM bright/positron/liberty `airport` layer, all three styles — is point-placed); (3) `icon-text-fit` + `text-optional: true`, where _pairFitBox is populated during layout BEFORE placement resolves, so a fitted icon whose text is then dropped keeps stretching to a text box that is not drawn. Non-constant form warns.",
    source: 'text-stage.ts isTextOptional -> getActiveTextPairKeys + the cascade stamp',
  },
  {
