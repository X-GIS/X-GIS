...
export const TOP_LEVEL: readonly CoverageEntry[] = [
  ...
  },
  {
    ...
    status: 'partial',
    impact: 'medium',
    note: "Mapbox v3 style-import. #2471 lowered it onto the wire that already existed: each entry's `url` is emitted as an xgis `import \"<url>\"` statement at the HEAD of the converted source, and resolveImportsAsync (module/resolver.ts:249) fetches it through the host's SSRF-guarded reader, runs parseModuleSource (:365) — which detects a Mapbox style and re-enters this converter — and splices the result, recursively, with cycle detection and a resolved-once memo. Emit position is draw order: the imports precede the style's own sources/layers, so an imported basemap lands UNDER the root style's layers, which is Mapbox's default placement absent a `slot`. Before this the field rode the lumped `Top-level style fields ignored:` warning beside `models`, which for a v3 Standard style meant losing the entire basemap in the same sentence as a minor field. PARTIAL, not supported, for three named residuals, each of which warns per-entry rather than as a lump: a `mapbox://` url (needs the Mapbox API + an access token, same gap as the sprite/glyphs check); an inline `data` style object in place of a url (the resolver reads by PATH, so there is nothing to fetch — converting it in place is a separate increment); and `config`, whose per-import option switches have no X-GIS equivalent and are dropped while the import itself still resolves. Layer `slot` — the v3 mechanism for ordering a root style's layers against an import's — is separately unhandled and only becomes observable now that imports resolve.",
    source: 'mapbox-to-xgis.ts (imports emit) -> module/resolver.ts resolveImportsAsync',
  },
]
