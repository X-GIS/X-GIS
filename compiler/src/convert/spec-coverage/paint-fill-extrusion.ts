...
export const PAINT_FILL_EXTRUSION: readonly CoverageEntry[] = [
  ...
  },
  {
    ...
    status: 'supported',
    impact: 'low',
    note: 'Stage 2 landed iter-186 2026-05-20. New `fillPipelinePatternExtruded` + Fallback variants (vs_main_ecef_extruded vertex + the POLYGON_EXTRUDED `extrudedVertexLayout` that buildPatternFillMaterials twins for per-feature z + fs_fill_pattern fragment). VTR routes extruded pattern shows via setPatternExtrudedPipelines + an extrudedPatternActive gate symmetric with the iter-183 ground path. Same world-anchored UV math as fill-pattern + line-pattern (abs_merc / repeat_m). Documented Stage 2 trade-off: pattern-extrude shows lose the per-fragment wall_shade lighting — sprite colour replaces the shaded fill rgb directly. Stage 2.1 (dedicated fs_fill_pattern_extruded that multiplies the sample by wall_shade) is a follow-up refinement. Constant string form supported end-to-end. #2380 — a per-feature `match()` over sprite names is ALSO supported via the same one-sublayer-per-sprite split as fill-pattern. The open-ended `["get", …]` form stays declined with its warning. iter-165 probe: ZERO uses in OFM bright/liberty target fixtures — Stage 2 is insurance for other styles.',
