...
export const EXPRESSIONS: readonly CoverageEntry[] = [
  ...
  },
  {
    ...
    status: 'partial',
    impact: 'low',
    note: "Numeric-valued AND hex-colour-valued interpolates densify at compile time into a piecewise-linear approximation (6 samples per segment, CSS bezier-eased via Newton-Raphson through cssBezierEase; colour stops sampled in sRGB at the eased fraction). BOTH axes do it: the data-driven densifier (isBezier, expr-interpolate.ts) and its zoom-axis twin (interpolateZoomStops, paint-helpers.ts), which gained the colour branch in #2166 — before that a zoom-axis colour ramp emitted the same two stops a ['linear'] ramp did, discarding the authored curve with only a warning. Runtime sees a longer linear stop list and visually approximates the bezier. PARTIAL for two residuals: expression-valued (non-literal) stops still warn and fold to pure linear (an eased sample cannot be computed until feature eval), and on the zoom axis an alpha-carrying 4-/8-digit hex folds too, because parseSrgbHex drops alpha and densifying would delete it. Iter 60-62 + colour-stop landing + #2166 zoom axis.",
