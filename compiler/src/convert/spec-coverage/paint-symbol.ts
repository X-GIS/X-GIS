...
export const PAINT_SYMBOL: readonly CoverageEntry[] = [
  ...
  },
  {
    ...
    status: 'partial',
    impact: 'low',
    note: 'CSS-px viewport offset for icons (independent of text-translate). Constant [dx, dy] form wired end-to-end: converter emits `label-icon-translate-{x,y}-N` (layers-symbol.ts) → LabelDef.iconTranslateX/Y → dispatchIcon adds it (× dpr) to the icon anchor before IconStage.addIcon (label-pass.ts), alongside icon-offset. Default [0,0] = no-op. #777 I-F: the per-feature EXPRESSION form (case/match/get → [dx,dy]) now lowers to `label-icon-translate-[<expr>]` → LabelDef.iconTranslateExpr → applyFeatureExprs evaluates it per feature into iconTranslateX/Y. #2166: the zoom-`interpolate` form no longer snaps. The tuple used to ride one opaque binding into the evaluator, whose interpolate builtin lerps only NUMERIC stop values and picks the closer stop for an array-valued one, so a zoom-animated offset jumped between stops. The converter now splits the vec2 per axis at convert time (isZoomInterpCandidate pre-gate + vec2AxisZoomInterp, the same decomposition addFillTranslate applies to fill-/line-translate) and re-pairs the two scalar zoom interpolates with an array literal inside the SAME binding — applyFeatureExprs already carries the camera zoom, so each axis resolves componentwise per dispatch with no runtime change. Both the modern expression and the legacy `{"stops": [[zoom, [dx, dy]], …]}` object lift through that one pre-gate, and `["exponential", base]` keeps its curve per axis. Still PARTIAL for one residual sub-form: an interpolate whose INPUT is a feature value rather than the zoom (e.g. `["interpolate", ["linear"], ["get", "t"], …]` over [dx,dy] stops) stays on the whole-tuple binding and still snaps to the closer stop — the per-axis split is only defined for the zoom-driven shape the shared lift understands.',
