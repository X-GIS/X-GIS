...
export const PAINT_LINE: readonly CoverageEntry[] = [
  ...
  },
  {
    ...
    status: 'supported',
    impact: 'low',
    note: 'Stage 2 landed iter-185 2026-05-20. line-renderer declares sprite_atlas at binding 5 + sprite_samp at binding 6 (shared TileBindGroupLayout with VTR so iter-181/182 atlas binding is already attached). New `fs_line_pattern` fragment + `pipelinePattern` alpha-blend pipeline. Pattern shows route via drawSegments(…, translucent, patternActive=true). World-anchored UV (abs_merc / repeat_m) — Stage 2.1 along-line UV (arc length + transverse v) is a follow-up refinement. UV bbox packed into stroke_color uniform slot (20-23); repeat metres packed into layer.color.r / .a via writeLayerSlot override. Constant string form supported end-to-end. #2380 — a per-feature `match()` over sprite names is ALSO supported via the same one-sublayer-per-sprite split as fill-pattern. The open-ended `["get", …]` form stays declined with its warning. iter-165 probe: ZERO line-pattern uses in OFM bright/liberty target fixtures, so visual A/B unavailable against current set — Stage 2 is insurance for other styles (USA OSM / custom sprites).',
