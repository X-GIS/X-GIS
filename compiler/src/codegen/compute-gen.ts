...
export function emitMatchComputeKernel(spec: MatchEmitSpec): ComputeKernel {
  ...
  // override), so it stays a hand-built const-array string. The sub-threshold path
  // routes through the shader-dsl IR (matchExpr → WGSL switch), inheriting cse /
  // const-fold / dce + the std430 layout — no more hand-assembled if-else chain.
  const useLut = sortedPatterns.length >= (readMatchLutThresholdOverride() ?? MATCH_LUT_THRESHOLD)
  const wgsl = useLut
    ? emitMatchLutWgsl(spec, sortedPatterns, armByPattern)
...
/** The P5 const-array LUT match kernel (arms ≥ MATCH_LUT_THRESHOLD): one
 *  `vec4<f32>` per arm, `LUT[id]` O(1) access. A deliberate GPU-perf strategy
 *  kept as hand-built WGSL OUTSIDE the DSL IR path (the LUT's const-array isn't an
 *  IR concept yet, and _perf-compute-strategy.spec A/Bs this exact emission). Byte-
 *  identical to the pre-IR-migration output. WGSL const arrays cap at 16384 elems
 *  (428 demotiles arms fine); multi-thousand arms would need a storage-buffer LUT. */
function emitMatchLutWgsl(
  spec: MatchEmitSpec,
  ...
  // The const LUT — one vec4<f32> per arm, indexed by sorted-pattern position
  // (matches the packer's ID). Baked into the module: no upload, no extra binding.
  lines.push(`const LUT: array<vec4<f32>, ${sortedPatterns.length}> = array<vec4<f32>, ${sortedPatterns.length}>(`)
  for (let i = 0; i < sortedPatterns.length; i++) {
    ...
    lines.push(`  vec4<f32>(${fmt(r)}, ${fmt(g)}, ${fmt(b)}, ${fmt(a)})${comma}`)
  }
  lines.push(');')
  lines.push('')
  ...
  // O(1) LUT branch — clamp the ID to [0, N); any out-of-range value (the packer's
  // unknown/sentinel id) falls to the explicit default colour.
...
function emitComputeKernelWgsl(
  ...
  return emitModule(module({ bindings: [featData.binding, outColor.binding, uCount.binding], funcs: [kernel] }))
}
