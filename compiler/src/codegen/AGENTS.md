...
| `node-types.ts` | Permanent compiler-side DSL vocabulary: the `Expr` union (lit, constref, param, varref, unop, binop, compare, logical, call, member, construct, select, index, matchExpr, rawString ops), the `ShaderType` union, and the `NodeLike<K>` seam interface with phantom WGSL-key brand. Now IMPORTS `Expr`/`ShaderType` from `@xgis/shader-dsl` (Tier-2 dedup landed — docs/architecture/package-responsibilities.md §5); the only compiler-local addition is the `rawString` op. `rawString` is a back-compat wrapper carrying a pre-built WGSL string; new emit sites use `_util/node-builders.ts` instead. |
| `node-to-wgsl.ts` | `nodeToWgslString(node)` lowers a `NodeLike` to a WGSL string by DELEGATING to the package emitter (`emitExpr` from `@xgis/shader-dsl`); only the compiler-local `rawString` op is unwrapped here. Used in tests to assert Node-emit equality. |
| `wgsl-expr.ts` | AST → shader-dsl IR Node compiler. `astToNode(expr, fieldMap, fnEnv?)` is the central tree-walk: it maps field accesses to the `feat_data[input.feat_id * stride + offset]` lookup Node, translates arithmetic/comparison/logical operators (comparisons bridge to f32 via `select`), handles WGSL builtins and user-defined function inlining, and supports pipe/conditional expressions — all via `_util/node-builders.ts` factories rather than hand-built strings. `exprToWGSL(expr, fieldMap, fnEnv?)` is now a back-compat string wrapper delegating to `astToNode` then `nodeToWgslString`. `collectFields` extracts the field name set for storage-buffer layout. |
...
- `node-types.ts` `Expr`/`ShaderType` are IMPORTED from `@xgis/shader-dsl` (the Tier-2 dedup landed — `compiler → @xgis/shader-dsl` is acyclic, the same shape as `compiler → @xgis/shared`; runtime already imports it). The only compiler-local addition is the `rawString` op. Remaining dedup (compute-gen kernels onto the IR builder) is tracked in docs/architecture/package-responsibilities.md §5.
...
- `node-to-wgsl.test.ts` exercises `nodeToWgslString` (which now delegates to the package `emitExpr`) over hand-typed IR fixtures — so it asserts the package's WGSL emit for the shared ops plus the compiler-local `rawString` unwrap.
