...
| `node-types.ts` | Permanent compiler-side DSL vocabulary: the `Expr` union (lit, constref, param, varref, unop, binop, compare, logical, call, member, construct, select, index, matchExpr, rawString ops), the `ShaderType` union, and the `NodeLike<K>` seam interface with phantom WGSL-key brand. A hand copy that mirrors `@xgis/shader-dsl`'s `shader-dsl/src/core/ir/nodes.ts` Expr union (extraction debt — dedup tracked in docs/architecture/package-responsibilities.md §5). `rawString` is a compiler-local back-compat wrapper that carries a pre-built WGSL string; new emit sites use `_util/node-builders.ts` instead. |
| `node-to-wgsl.ts` | Compiler-side hand copy of `@xgis/shader-dsl`'s WGSL emit walk. `nodeToWgslString(node)` lowers a `NodeLike` to a WGSL string; used in tests to assert Node-emit equality. NOTE: `node-to-wgsl.test.ts` pins it only to ITSELF (hand-typed fixtures) — it does NOT import the package emitter, so it cannot catch drift. |
...
- `node-types.ts` `Expr` union is a HAND COPY of `@xgis/shader-dsl`'s `shader-dsl/src/core/ir/nodes.ts` and must mirror it. The old "cannot import — rootDir / circular workspace dep" reason is STALE: `@xgis/shader-dsl` is a zero-dep leaf, so `compiler → @xgis/shader-dsl` is acyclic (like `compiler → @xgis/shared`; runtime already imports it). Until the dedup lands (docs/architecture/package-responsibilities.md §5), keep the copy in sync by hand — `node-to-wgsl.test.ts` does NOT guard it (it pins the copy to itself).
...
- `node-to-wgsl.test.ts` is a SELF-consistency oracle: its fixtures are hand-typed to the local `node-types.ts` shape, so it pins `nodeToWgslString` to itself. It does NOT import `@xgis/shader-dsl` and therefore does NOT certify compiler ↔ package emit parity (that guarantee requires the §5 dedup or a test that imports the package emitter).
