...
describe('s102CatalogueDocument → what the ENGINE reads (#1453)', () => {
  ...
  it('the engine parses it, keyed by the S-100 cell name', () => {
    const items = parseCoverageCatalogue(doc(), '/opendata/s102/catalog.json', 'test')
  ...
  it('hrefs are RELATIVE, so they follow the document to whichever origin served it', () => {
    ...
    const dev = parseCoverageCatalogue(doc(), '/opendata/s102/catalog.json', 'test')
    expect(dev[0]!.href).toBe(
      '/opendata/s102/cells/ed3.0.0/Southeast/Wilmington/102US004SC1EV262227.h5',
    )
    const prod = parseCoverageCatalogue(
      doc(),
      'https://opendata-bridge.x-gis.workers.dev/opendata/s102/catalog.json',
      'test',
    )
    expect(prod[0]!.href).toBe(
      'https://opendata-bridge.x-gis.workers.dev/opendata/s102/cells/ed3.0.0/Southeast/Wilmington/102US004SC1EV262227.h5',
    )
  })
