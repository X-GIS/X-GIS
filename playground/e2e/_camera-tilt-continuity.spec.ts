...
const PROJECTIONS = [
  'mercator', 'globe', 'equirectangular', 'natural_earth',
  'orthographic', 'stereographic', 'azimuthal_equidistant', 'oblique_mercator',
]

test('camera/globe tilt continuity — fixed point moves smoothly under pitch/bearing', async ({ page }) => {
  test.setTimeout(180_000)
  const errs: string[] = []
  page.on('pageerror', e => errs.push(String(e)))
  await page.setViewportSize({ width: 800, height: 800 })
  await page.goto('/debug-labels.html?style=openfreemap-bright#4/30/127', { waitUntil: 'domcontentloaded' })
  ...
  const reports = await page.evaluate(async () => {
    ...
    const setCam = (zoom: number, bearing: number, pitch: number) => {
      const c = m.getCamera()
      c.zoom = zoom; c.centerX = 127 * RAD * R
      c.centerY = Math.log(Math.tan(Math.PI / 4 + 30 * RAD / 2)) * R
      c.bearing = bearing; c.pitch = pitch
    ...
    const sweep = async (kind: 'pitch' | 'bearing', from: number, to: number): Promise<number> => {
      ...
      for (let i = 0; i <= N; i++) {
        ...
        const n = ndc(M, 0, D, 0), e = ndc(M, D, 0, 0)
        traj.push([n[0], n[1], e[0], e[1]])
      }
      const deltas: number[] = []
      for (let i = 1; i < traj.length; i++) {
        let s = 0
        for (let k = 0; k < 4; k++) { const d = traj[i]![k] - traj[i - 1]![k]; s += d * d }
    ...
    for (const proj of ['mercator', 'globe', 'equirectangular', 'natural_earth', 'orthographic', 'stereographic', 'azimuthal_equidistant', 'oblique_mercator']) {
      m.setProjection(proj); await sleep(400)
      out.push({
        proj,
  ...
  for (const r of reports) {
    console.log(`[${r.proj}] pitch[2,70] NDC max/median=${r.pitch.toFixed(1)} bearing=${r.bearing.toFixed(1)} | pitch0→2=${r.pitch0.toFixed(1)} (known azimuthal z0 case, logged)`)
