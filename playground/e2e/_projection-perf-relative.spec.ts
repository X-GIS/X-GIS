...
async function setProjAndSettle(
...
test('relative projection perf — no projection is dramatically slower than Mercator', async ({
  page,
}) => {
  ...
  await page.goto('/debug-labels.html?style=openfreemap-bright#9/36.5/127.6', {
  ...
  for (const proj of PROJECTIONS) {
    ...
    console.log(
      `[${proj}] median=${s.median.toFixed(1)}ms p95=${s.p95.toFixed(1)}ms (${s.count} frames)`,
    )
  }
  ...
  for (const proj of PROJECTIONS) {
    ...
    if (medRatio > 4 || p95Ratio > 4)
      offenders.push(`${proj}: median ${medRatio.toFixed(1)}× p95 ${p95Ratio.toFixed(1)}×`)
  }
  console.log(
    `mercator baseline median=${base.median.toFixed(1)} p95=${base.p95.toFixed(1)}; offenders: ${offenders.length ? offenders.join(' | ') : 'none'}`,
  )
