...
test('worldwrap at z=0 sweep', async ({ page }) => {
    ...
    return await page.evaluate(() => {
      const win = window as unknown as {
        __xgisMap?: {
          ...
          vtSources?: Map<
            string,
            { renderer: { getDrawStats(): { missedTiles: number; tilesVisible: number } } }
          >
        }
      }
      ...
      const loadingSet = (
        m.rasterRenderer as unknown as { loadingTiles?: Map<unknown, unknown> } | undefined
      )?.loadingTiles
  ...
  const midMotionProbe = await page.evaluate(
    () =>
      new Promise<{
        minVis: number
        maxVis: number
        framesWithMissed: number
      }>((resolve) => {
        const win = window as unknown as {
          __xgisMap?: {
  ...
  const report = {
    ...
    consoleErrors,
    consoleWarns: consoleWarns.filter(
      (w) => !w.includes('[vite]') && !w.includes('powerPreference'),
    ),
  }
