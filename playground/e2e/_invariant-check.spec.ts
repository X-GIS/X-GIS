...
test.describe('Production invariant — visibility / fallback consistency', () => {
  ...
  test('pmtiles_layered cold-load → zoom-in → zoom-out → pan: no invariant violation', async ({
    page,
  }) => {
    ...
    await page.goto(`/demo.html?id=pmtiles_layered#10/37.5665/126.978/0/0`, {
    ...
    await page.waitForFunction(
      () => {
        ...
        for (const { renderer } of map.vtSources.values())
          cz = renderer._selection?._hysteresisZ ?? cz
        return cz >= 9
      },
      null,
      { timeout: 30_000 },
    ...
    // Phase B: zoom-out
    await page.evaluate(() => {
      window.__xgisMap!.camera!.zoom = 9
    })
    await page.waitForTimeout(3000)
    ...
    console.log(
      '[invariant-check] zero violations across cold-load + zoom-in/out + pan+rotate + high-pitch',
    )
  })
})
