...
test.describe('Continuous wheel-zoom: bounded backend state under sustained gesture', () => {
  ...
  test('5 s in + 5 s out: tilesVisible + heap stay bounded mid-gesture', async ({ page }) => {
    ...
    const result = await page.evaluate(
      async ({ startZoom, endZoom, durationMs, sampleEveryFrames }) => {
        const map = window.__xgisMap!
    ...
    const drive = async (startZoom: number, endZoom: number, durationMs: number) =>
      page.evaluate(
        async ({ startZoom, endZoom, durationMs }) => {
          const map = window.__xgisMap!
          const t0 = performance.now()
    ...
    if (cacheStats) {
      ...
      console.log(
        `  bufferPool:  ${cacheStats.bufferPoolHits}h / ${cacheStats.bufferPoolMisses}m = ${ratio(cacheStats.bufferPoolHits, cacheStats.bufferPoolMisses)}`,
      )
      console.log(
        `  gpuCache:    ${cacheStats.gpuCacheHits}h / ${cacheStats.gpuCacheMisses}m = ${ratio(cacheStats.gpuCacheHits, cacheStats.gpuCacheMisses)}`,
      )
      console.log(
        `  hasTileData: ${cacheStats.hasTileDataHits}h / ${cacheStats.hasTileDataMisses}m = ${ratio(cacheStats.hasTileDataHits, cacheStats.hasTileDataMisses)}`,
      )
    }
