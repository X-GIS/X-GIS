...
test('a raster tile renders on WebGl2Device through the engine loop (?forcegl2=1)', async ({ page }) => {
  await page.goto('/demo.html?id=minimal&forcegl2=1&e2e=1', { waitUntil: 'domcontentloaded' })
  await page.waitForFunction(() => (window as unknown as { __xgisReady?: boolean }).__xgisReady === true, { timeout: 20000 })
  await page.waitForTimeout(500)

  const r = await page.evaluate(() => {
    const w = window as unknown as { __xgisActiveBackend?: string; __xgisMap?: { ctx?: { rhi?: { backend?: string; gl?: WebGL2RenderingContext }; _validationErrors?: unknown[]; sampleCount?: number } } }
    ...
    for (let i = 1; i < N; i++) for (let j = 1; j < N; j++) {
      gl.readPixels(Math.floor((W * i) / N), Math.floor((H * j) / N), 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, px)
  ...
  const near = (p: number[], c: number[], tol = 70) => Math.abs(p[0] - c[0]) < tol && Math.abs(p[1] - c[1]) < tol && Math.abs(p[2] - c[2]) < tol
  const checker = r.samples.filter((p) => near(p, ON) || near(p, OFF)).length
  const black = r.samples.filter((p) => p[0] < 20 && p[1] < 20 && p[2] < 20).length
  expect(checker, `checker texels (orange/blue) ${checker}/${r.samples.length}`).toBeGreaterThan(r.samples.length * 0.6)
  expect(black, `near-black (clear) texels ${black}/${r.samples.length}`).toBeLessThan(r.samples.length * 0.2)
})
