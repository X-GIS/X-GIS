...
test('OFM Liberty world-copy probe — #0/0/-54/90/63', async ({ page }) => {
  ...
  await page.goto(
    `/demo.html?id=__import#0/0/-54/90/63`,
  ...
  const total = w * h
  let whiteish = 0, blackish = 0, blueish = 0, beigeish = 0, otherCount = 0
  for (let i = 0; i < total; i++) {
    const off = i * 4
    const r = png.data[off], g = png.data[off + 1], b = png.data[off + 2]
    if (r > 240 && g > 240 && b > 240) whiteish++
    else if (r < 16 && g < 16 && b < 16) blackish++
    else if (b > r + 20 && b > g + 20) blueish++       // water blue
    else if (r > 220 && g > 215 && b > 200) beigeish++ // Liberty bg #f8f4f0 ish
    else otherCount++
  }
  // eslint-disable-next-line no-console
  console.warn(`[Liberty z0 p63] ${w}x${h} white=${whiteish} (${(100*whiteish/total).toFixed(1)}%) blue=${blueish} (${(100*blueish/total).toFixed(1)}%) beige=${beigeish} (${(100*beigeish/total).toFixed(1)}%) black=${blackish} other=${otherCount} total=${total}`)
