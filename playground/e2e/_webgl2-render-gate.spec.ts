...
test('WebGL2 RHI render proof: WebGl2Device samples a 2×2 checker, readback matches', async ({ page }) => {
...
${result.fsGlsl}`
  ...
  expect(eqColor(result.bottomLeft, result.expected.bottomLeft), `bottom-left mismatch\n${ctx}`).toBe(true)
  expect(eqColor(result.bottomRight, result.expected.bottomRight), `bottom-right mismatch\n${ctx}`).toBe(true)
  ...
  for (const q of [result.bottomLeft, result.bottomRight, result.topLeft, result.topRight]) {
    expect(eqColor(q, result.expected.clear), `a quadrant read as the magenta CLEAR (quad did not draw)\n${ctx}`).toBe(false)
  }
})

test('WebGL2 RHI reflection BY NAME: a multi-texture group binds correctly regardless of order', async ({ page }) => {
  test.setTimeout(30_000)
  const errors: string[] = []
  page.on('pageerror', (e) => errors.push(e.message))
  await page.goto('/demo.html?id=minimal', { waitUntil: 'domcontentloaded' })
...
expected left(tex_a)=${r.expected.left} right(tex_b)=${r.expected.right}`
  ...
  expect(eqColor(r.named.right, r.expected.right), `named right should be tex_b (blue)\n${ctx}`).toBe(true)
  ...
  expect(eqColor(r.ordered.left, r.expected.right), `ordered left should SWAP to tex_b (blue)\n${ctx}`).toBe(true)
  expect(eqColor(r.ordered.right, r.expected.left), `ordered right should SWAP to tex_a (red)\n${ctx}`).toBe(true)
})

test('WebGL2 RHI renders a REAL DSL shader (overdraw-compose) — readback matches the colormap', async ({ page }) => {
  test.setTimeout(30_000)
  const errors: string[] = []
  page.on('pageerror', (e) => errors.push(e.message))
  await page.goto('/demo.html?id=minimal', { waitUntil: 'domcontentloaded' })
...
test('WebGL2 RHI STORAGE→data-texture: reads a storage array<f32> via emulation', async ({ page }) => {
  test.setTimeout(30_000)
  const errors: string[] = []
  page.on('pageerror', (e) => errors.push(e.message))
  await page.goto('/demo.html?id=minimal', { waitUntil: 'domcontentloaded' })
...
${r.fsGlsl}`
  ...
  const grey = (q: number[], v: number) => Math.abs(q[0] - v) <= 3 && Math.abs(q[1] - v) <= 3 && Math.abs(q[2] - v) <= 3
...
test('WebGL2 RHI MULTI-STORAGE: two storage buffers → two data textures, bound by name', async ({ page }) => {
  test.setTimeout(30_000)
  const errors: string[] = []
  page.on('pageerror', (e) => errors.push(e.message))
  await page.goto('/demo.html?id=minimal', { waitUntil: 'domcontentloaded' })
...
test('WebGL2 RHI STORAGE bitcast-u32 lanes (feat_data denormal-bits risk)', async ({ page }) => {
  ...
  expect(eq(r.px[1], 200), `bitcast DENORMAL lane (R32F flush risk): ${r.px} exp ${r.expected}`).toBe(true)
})
...
test('WebGL2 RHI STORAGE 2D-tiled: >maxTextureSize array wraps across rows', async ({ page }) => {
  ...
  expect(eqColor(r.px, r.expected, 3), `2D-tiled storage mismatch: ${r.px} exp ${r.expected}`).toBe(true)
})

test('WebGL2 RHI STORAGE strided lane: feat_data[base*STRIDE+lane] computed index', async ({ page }) => {
  test.setTimeout(30_000)
  const errors: string[] = []
  page.on('pageerror', (e) => errors.push(e.message))
  await page.goto('/demo.html?id=minimal', { waitUntil: 'domcontentloaded' })
...
test('WebGL2 RHI STORAGE array<Struct>: scalar-field lane read (shapes/segments path)', async ({ page }) => {
  test.setTimeout(30_000)
  const errors: string[] = []
  page.on('pageerror', (e) => errors.push(e.message))
  await page.goto('/demo.html?id=minimal', { waitUntil: 'domcontentloaded' })
...
test('WebGL2 RHI: the generic Material + executeItems render on WebGl2Device', async ({ page }) => {
  ...
  page.on('pageerror', (e) => errors.push(e.message))
  await page.goto('/demo.html?id=minimal', { waitUntil: 'domcontentloaded' })
  const r = await page.evaluate(async () => (await import('/e2e/_webgl2-proof.ts')).runWebgl2MaterialProof())
