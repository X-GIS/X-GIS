...
test('interaction audit — pitch, bearing, deep zoom', async ({ page }) => {
      ...
      () =>
        new Promise<{
          ...
          function tick() {
            ...
            if (missed > 0) {
              consecMissed++
              sustainedMissed = Math.max(sustainedMissed, consecMissed)
            } else consecMissed = 0
  ...
  for (const r of results) {
    ...
    const status = r.consoleErrors.length > 0 ? '✗' : extra.restMissedFrames > 30 ? '⚠' : '✓'
    console.log(
      `${status} ${r.id.padEnd(36)} p95=${r.p95Dt}ms max=${r.maxDt}ms moveMissed=${r.maxMissedTiles} REST=${extra.restMissedFrames}frames/peak=${extra.restMissedPeak} flickerEv=${r.flickerEvents} errors=${r.consoleErrors.length}`,
    )
