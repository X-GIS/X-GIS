...
test.describe('Mapbox import hook — inline GeoJSON auto-push', () => {
  ...
  test('__xgisImportMapbox renders inline FeatureCollection without manual setSourceData', async ({ page }) => {
    ...
    await page.waitForFunction(
      () => typeof (window as unknown as { __xgisImportMapbox?: unknown }).__xgisImportMapbox === 'function',
      null, { timeout: 30_000 },
    )
    ...
    const result = await page.evaluate(async ({ fill, bg, tol }) => {
      ...
      return {
        ...
        meanRGB: [Math.round(sums[0] / total), Math.round(sums[1] / total), Math.round(sums[2] / total)] as [number, number, number],
      }
    ...
    console.log(`[import-inline-button] fill=${(result.fillFraction * 100).toFixed(2)}% mean=rgb(${result.meanRGB.join(',')})`)
    ...
    const realErrors = errors.filter(e =>
      !e.includes('vite/dist/client') && !e.includes('[X-GIS pass:'))
