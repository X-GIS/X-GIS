...
export function surveyGlslEmit(): SurveyRow[] {
  ...
  for (const c of cases) {
    ...
    try {
      m = c.build() as Parameters<typeof emitGlslModule>[0]
    } catch (e) {
      rows.push({
        name: c.name,
        vertex: 'BUILD THROW: ' + (e as Error).message,
        fragment: '',
        link: '-',
      })
      continue
    ...
    const emit = (stage: 'vertex' | 'fragment') => {
      try {
        const s = emitGlslModule(m, stage)
        if (stage === 'vertex') vsSrc = s
        else fsSrc = s
    ...
    const vertex = emit('vertex'),
      fragment = emit('fragment')
    ...
    if (gl && vsSrc && fsSrc) {
      ...
      if (!vs.ok) link = 'VS COMPILE FAIL: ' + vs.log.slice(0, 300)
        ...
        link = gl.getProgramParameter(prog, gl.LINK_STATUS)
          ? 'LINK OK'
          : 'LINK FAIL: ' + (gl.getProgramInfoLog(prog) ?? '').slice(0, 300)
      }
    }
