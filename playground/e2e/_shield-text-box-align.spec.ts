...
test('shield text ink-centre coincides with its box centre', async ({ page }) => {
  ...
  await page.goto('/debug-labels.html?style=openfreemap-bright#11.14/37.54621/126.89708', {
    ...
    const m = (
      window as unknown as {
        __xgisMap: {
          ...
          setIconDumpEnabled(b: boolean): void
          getDumpedIcons(): DI[] | null
          invalidate?(): void
        }
      }
    ).__xgisMap
    m.setLabelDumpFilter('')
    m.setIconDumpEnabled(true)
    m.invalidate?.()
  ...
  for (const g of out.gaps) {
    ...
    if (rel > worst) {
      worst = rel
      worstText = `"${g.text}" gap=${g.gap.toFixed(1)}px fs=${g.fs}`
    }
  }
  console.log(
    `worst text↔box gap: ${(worst * 100).toFixed(0)}% of fontSize (${worstText}); pageErrors: ${errs.length}`,
  )
