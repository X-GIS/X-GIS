...
test('map.project(): center→canvas-centre, lon offset monotone', async ({ page }) => {
  ...
  await page.goto('/demo.html?id=minimal#3/20.0/0.0', { waitUntil: 'domcontentloaded', timeout: 60_000 })
  await page.waitForFunction(() => (window as unknown as { __xgisReady?: boolean }).__xgisReady === true, null, { timeout: 60_000 }).catch(() => {})
