...
type W = { __xgisReady?: boolean; __xgisMap?: { invalidate?: () => void; setGlyphsUrl?: (u: string) => void }; __xgisTextViaRhi?: boolean }
...
test('text RHI parity: legacy vs RHI path pixel-identical', async ({ page }) => {
  ...
  page.on('console', (m) => { if (m.type() === 'error') errors.push(m.text()); if (/TEXTRHI/.test(m.text())) logs.push(m.text()) })

  await page.setViewportSize({ width: 700, height: 700 })
  await page.goto('/demo.html?id=openfreemap_bright&e2e=1#13/35.681/139.767', { waitUntil: 'domcontentloaded' })
  ...
  await page.evaluate(() => { const w = window as unknown as W; w.__xgisTextViaRhi = false; w.__xgisMap?.invalidate?.() })
  ...
  await page.evaluate(() => { const w = window as unknown as W; w.__xgisTextViaRhi = true; w.__xgisMap?.invalidate?.() })
  await page.waitForTimeout(800)
