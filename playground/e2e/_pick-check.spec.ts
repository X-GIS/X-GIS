...
for (const id of ['picking_demo', 'fixture_picking']) {
  test(`pick-check: ${id}`, async ({ page }) => {
    ...
    await page.addInitScript(() => { (window as unknown as { __xgisDrawOrderTrace: unknown[] }).__xgisDrawOrderTrace = [] })
    ...
    if (id === 'picking_demo') await page.locator('canvas').first().screenshot({ path: 'D:/X-GIS/tmp/shots/n460_z05.png' })
    const result = await page.evaluate(async () => {
      const m = (window as unknown as { __xgisMap?: { pickAt?: (x: number, y: number) => Promise<unknown>; getCamera?: () => { zoom: number; centerX: number; centerY: number }; markCameraPositioned?: () => void } }).__xgisMap
      if (!m || typeof m.pickAt !== 'function') return { hasPickAt: false, hits: [] as unknown[], zoomedHits: [] as unknown[] }
      const hits: unknown[] = []
      for (let gx = 150; gx <= 750; gx += 100) {
        for (let gy = 120; gy <= 520; gy += 100) {
          try { const r = await m.pickAt(gx, gy); if (r) hits.push({ gx, gy, r }) } catch (e) { hits.push({ gx, gy, err: String(e) }) }
        }
      }
      // #460: is the z0 fill DRAWN at z0.5? Dump the draw trace.
      const w = window as unknown as { __xgisDrawOrderTrace?: unknown[] }
      hits.push({ __z05_drawtrace: Array.isArray(w.__xgisDrawOrderTrace)
      ...
      if (m.getCamera && m.markCameraPositioned) {
        const cam = m.getCamera(); const R = 6378137
        cam.zoom = 3; cam.centerX = 20 * Math.PI / 180 * R
        ...
        for (let gx = 300; gx <= 600; gx += 75) {
          for (let gy = 200; gy <= 450; gy += 75) {
            try { const r = await m.pickAt(gx, gy); if (r) zoomedHits.push({ gx, gy, r }) } catch {}
          }
        }
      }
      return { hasPickAt: true, hits, zoomedHits }
    })
    if (id === 'picking_demo') await page.locator('canvas').first().screenshot({ path: 'D:/X-GIS/tmp/shots/n460_z3.png' })
    console.log(`  zoomedHits=${(result as { zoomedHits: unknown[] }).zoomedHits.length} ${JSON.stringify((result as { zoomedHits: unknown[] }).zoomedHits.slice(0, 4))}`)
