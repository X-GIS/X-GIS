...
type W = { __xgisReady?: boolean; __xgisMap?: { invalidate?: () => void }; __xgisPointViaRhi?: boolean }
...
test('point RHI parity: legacy vs RHI path pixel-identical', async ({ page }) => {
  ...
  page.on('console', (m) => { if (m.type() === 'error') errors.push(m.text()); if (/POINTRHI/.test(m.text())) logs.push(m.text()) })
  ...
  await page.evaluate(() => { const w = window as unknown as W; w.__xgisPointViaRhi = false; w.__xgisMap?.invalidate?.() })
  await page.waitForTimeout(800)
  writeFileSync(join(OUT, 'point-parity-legacy.png'), await page.locator('#map').screenshot())

  await page.evaluate(() => { const w = window as unknown as W; w.__xgisPointViaRhi = true; w.__xgisMap?.invalidate?.() })
  await page.waitForTimeout(800)
