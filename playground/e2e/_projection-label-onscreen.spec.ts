...
const PROJECTIONS = [
  'mercator', 'globe', 'orthographic', 'azimuthal_equidistant',
  'stereographic', 'equirectangular', 'natural_earth', 'oblique_mercator',
]
...
test('per-projection labels stay on-screen (no wrong-tile mispositioning)', async ({ page }) => {
  ...
  await page.goto('/debug-labels.html?style=openfreemap-bright#14/37.566/126.978', { waitUntil: 'domcontentloaded' })
  await page.waitForFunction(() => !!(window as unknown as { __xgisMap?: unknown }).__xgisMap, null, { timeout: 30_000 })
  await page.waitForTimeout(8_000)

  const reports = await page.evaluate(async ({ W, H, MARGIN }) => {
    interface DL { anchorX: number; anchorY: number; curved: boolean; text: string }
    const m = (window as unknown as {
      __xgisMap: {
        ...
        markCameraPositioned(): void; invalidate?(): void
    ...
    const setCam = (zoom: number, pitch: number) => {
      const c = m.getCamera()
      c.zoom = zoom; c.centerX = 126.978 * RAD * R
      c.centerY = Math.log(Math.tan(Math.PI / 4 + 37.566 * RAD / 2)) * R
      c.bearing = 0; c.pitch = pitch
    ...
    const settle = async () => {
      ...
      for (let i = 0; i < 40; i++) {
        ...
        if (n === 0) { if (++st >= 2) return } else st = 0
      }
    }
    const out: { proj: string; total: number; offscreen: number; worst: string }[] = []
    for (const proj of ['mercator', 'globe', 'orthographic', 'azimuthal_equidistant', 'stereographic', 'equirectangular', 'natural_earth', 'oblique_mercator']) {
      m.setProjection(proj)
      setCam(14, 0)
      await settle()
      m.setLabelDumpFilter(''); m.invalidate?.(); await sleep(450)
      ...
      for (const l of labels) {
        ...
        if (outX || outY) {
          ...
          if (d > worstD) { worstD = d; worst = `"${l.text.replace(/\n/g, '\\n')}"@(${ax.toFixed(0)},${ay.toFixed(0)})` }
        }
      }
  ...
  for (const r of reports) {
    console.log(`[${r.proj}] ${r.total} labels, ${r.offscreen} off-screen${r.worst ? ' worst ' + r.worst : ''}`)
  ...
  expect(offenders, 'projections with many off-screen label anchors (wrong-tile mispositioning)').toEqual([])
})
