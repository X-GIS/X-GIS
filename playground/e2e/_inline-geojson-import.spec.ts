...
test.describe('Mapbox style import — inline GeoJSON auto-push', () => {
  ...
  test('inline FeatureCollection renders without a manual setSourceData call', async ({ page }) => {
    ...
    // variance.
    await page.goto('/demo.html?id=import_mapbox_inline_geojson#7/37.5/127', {
    ...
    console.log(
      `[inline-geojson] fill=${(result.fillFraction * 100).toFixed(2)}% bg=${(result.bgFraction * 100).toFixed(2)}% mean=rgb(${result.meanRGB.join(',')})`,
    )
