...
const ON: [number, number, number] = [240, 80, 40] // checker orange (ocean/underlay)
const OFF: [number, number, number] = [30, 30, 120] // checker blue
...
test('ne_110m country fills render on WebGl2Device (?forcegl2=1)', async ({ page }) => {
  ...
  expect(r.checker, `checker (ocean) pixels ${r.checker}/${r.total}`).toBeGreaterThan(
    r.total * 0.2,
  )
})
