  ...
  test('pmtiles_layered: zoom 3 → 6 over ~30 ticks, capture per-frame state', async ({ page }) => {
    ...
    for (let i = 0; i <= 30; i += SCREENSHOT_INTERVAL) {
      ...
      writeFileSync(
        join(
          OUT_DIR,
          `frame-${String(i).padStart(3, '0')}-z${(3.0 + (3.0 / 30) * i).toFixed(2)}.jpg`,
        ),
        buf,
      )
    }
    ...
    console.log(
      `  Recovery to 0 pending:  ${recoveryMs >= 0 ? `${recoveryMs.toFixed(0)} ms after final zoom step` : 'NEVER (still pending at end of capture)'}`,
    )
    console.log(`  Per-VTR final state:`)
    for (const v of timeline.at(-1)!.vtrs) {
      console.log(
        `    ${v.sourceName.padEnd(20)} tilesVisible=${v.tilesVisible} missed=${v.missedTiles} gpu=${v.gpuCacheCount} pending=${v.pendingUploads}`,
      )
    }
