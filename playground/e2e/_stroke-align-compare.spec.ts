// Compare stroke-outset, stroke-inset, default-center rendering at
// a zoomed-in corner. If only stroke-outset shows the gap-at-corner,
// the bug is asymmetric in the offset sign handling.
...
const CASES = ['fixture_stroke_outset', 'fixture_stroke_inset', 'fixture_stroke_fill']

for (const id of CASES) {
  test(`align-compare: ${id}`, async ({ page }) => {
    ...
    writeFileSync(join(ART, `${id}-corner.png`), Buffer.from(cropped, 'base64'))
  })
}
