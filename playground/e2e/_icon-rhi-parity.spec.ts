...
type W = { __xgisReady?: boolean; __xgisMap?: { invalidate?: () => void }; __xgisIconViaRhi?: boolean }
...
test('icon RHI parity: legacy vs RHI path pixel-identical', async ({ page }) => {
  ...
  page.on('console', (m) => { if (m.type() === 'error') errors.push(m.text()); if (/ICONRHI/.test(m.text())) logs.push(m.text()) })

  await page.setViewportSize({ width: 600, height: 600 })
  await page.goto('/demo.html?id=openfreemap_bright&e2e=1#15.5/35.681/139.767', { waitUntil: 'domcontentloaded' })
  ...
  await page.evaluate(() => { const w = window as unknown as W; w.__xgisIconViaRhi = false; w.__xgisMap?.invalidate?.() })
  ...
  await page.evaluate(() => { const w = window as unknown as W; w.__xgisIconViaRhi = true; w.__xgisMap?.invalidate?.() })
  await page.waitForTimeout(800)
