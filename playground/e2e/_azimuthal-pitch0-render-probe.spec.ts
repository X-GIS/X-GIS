...
async function setProjPitchSettle(page: import('@playwright/test').Page, proj: string, pitch: number): Promise<void> {
  await page.evaluate(({ proj, pitch }) => {
...
test('azimuthal-family pitch-0 render-path continuity (real vs snapshot artifact)', async ({ page }) => {
  test.setTimeout(220_000)
  const errs: string[] = []
  page.on('pageerror', e => errs.push(String(e)))
  await page.setViewportSize({ width: 800, height: 800 })
  await page.goto('/debug-labels.html?style=openfreemap-bright#2/30/127', { waitUntil: 'domcontentloaded' })
  ...
  for (const proj of SPHERE_PROJS) {
    ...
    console.log(`[${proj}] 0→0.5°=${(dBoundary * 100).toFixed(2)}% 30→30.5°=${(dMid * 100).toFixed(2)}% slope-ratio=${ratio.toFixed(1)}× ⇒ ${verdict}`)
    if (ratio > 5) verdicts.push(`${proj}: ${ratio.toFixed(1)}× (${(dBoundary * 100).toFixed(1)}% vs ${(dMid * 100).toFixed(1)}%)`)
  }
  console.log(`REAL render-path pitch-0 jumps: ${verdicts.length ? verdicts.join(' | ') : 'NONE — gate E finding is a snapshot-matrix artifact'}; pageErrors: ${errs.length}`)
