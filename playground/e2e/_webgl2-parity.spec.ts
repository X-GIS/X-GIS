...
const FIXTURES = (
  process.env.XGIS_GL2_FIXTURES ?? 'fixture_raster_local,fixture_square,fixture_triangle'
)
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean)
...
async function shoot(
  ...
  await page.goto(`/demo.html?id=${id}&e2e=1${gl2 ? '&forcegl2=1' : ''}`, {
    waitUntil: 'domcontentloaded',
  })
  await page.waitForFunction(() => (window as unknown as ReadyWin).__xgisReady === true, null, {
    timeout: 35_000,
  })
        ...
        () =>
          new Promise<string>((resolve, reject) => {
            ...
            if (!c || typeof c.toBlob !== 'function')
              return reject(new Error('#map canvas missing'))
            c.toBlob((b) => {
  ...
  if (errs.length)
    console.log(`  [${id} gl2=${gl2}] ${errs.length} page error(s): ${errs[0]?.slice(0, 100)}`)
  return png
}
...
test('WebGL2 ↔ WebGPU parity (local, real-GPU)', async ({ page }) => {
  ...
  for (const id of FIXTURES) {
    ...
    const ndiff = pixelmatch(webgpu.data, webgl2.data, diff.data, webgpu.width, webgpu.height, {
      threshold: 0,
    })
    let maxDelta = 0
    for (let i = 0; i < webgpu.data.length; i += 4) {
      for (let c = 0; c < 3; c++)
        maxDelta = Math.max(maxDelta, Math.abs(webgpu.data[i + c] - webgl2.data[i + c]))
    }
