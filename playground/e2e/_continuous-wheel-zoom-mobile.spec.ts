...
test.describe('Mobile continuous wheel-zoom', () => {
  ...
  test('5 s in + 5 s out at 390×844: peak tilesVisible bounded', async ({ page }) => {
    ...
    console.log(
      `[zoom-in continuous] tilesVisible peak ${inPeak.tilesVisible}, drawCalls peak ${inPeak.drawCalls}`,
    )
    const outPeak = await drive(17, 12, 5000)
    console.log(
      `[zoom-out continuous] tilesVisible peak ${outPeak.tilesVisible}, drawCalls peak ${outPeak.drawCalls}`,
    )
