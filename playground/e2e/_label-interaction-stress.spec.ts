...
test('label interaction stress — Korea bilingual, no collapse under motion', async ({ page }) => {
  ...
  await page.goto('/debug-labels.html?style=openfreemap-bright#7/37.0/127.5', { waitUntil: 'domcontentloaded' })
  ...
  const reports = await page.evaluate(async () => {
    ...
    interface DL { text: string; fontSize: number; curved: boolean; glyphs: DG[] }
    ...
    const analyze = async (): Promise<{ total: number; bad: { text: string; why: string }[] }> => {
      m.setLabelDumpFilter(''); m.invalidate?.()
      await sleep(450)
      ...
      for (const l of labels) {
        ...
        for (const k of ks) {
          ...
          if (Math.max(...rys) - Math.min(...rys) > l.fontSize * 0.6) why = `렌더Y이탈 ${(Math.max(...rys) - Math.min(...rys)).toFixed(0)}`
        }
        for (let i = 1; i < lineRY.length; i++) if (lineRY[i]! <= lineRY[i - 1]! + 1) why = `줄겹침 ${lineRY[i]!.toFixed(0)}<=${lineRY[i - 1]!.toFixed(0)}`
        if (why) bad.push({ text: l.text.replace(/\n/g, '\\n'), why })
      }
      return { total: labels.length, bad }
    }
    type Phase = [string, [number, number, number, number, number], [number, number, number, number, number]]
    const phases: Phase[] = [
    ...
    for (const [name, s, e] of phases) {
      ...
      for (let i = 0; i <= steps; i++) {
        const t = i / steps
        setView(s[0] + (e[0] - s[0]) * t, s[1] + (e[1] - s[1]) * t, s[2] + (e[2] - s[2]) * t, s[3] + (e[3] - s[3]) * t, s[4] + (e[4] - s[4]) * t)
        await sleep(110)
      }
  ...
  for (const r of reports) {
    console.log(`[${r.phase}] ${r.total} labels, ${r.bad.length} bad` + (r.bad.length ? ': ' + r.bad.map(b => `"${b.text}"(${b.why})`).join(', ') : ''))
