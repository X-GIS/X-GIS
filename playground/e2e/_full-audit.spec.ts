...
test('full demo audit', async ({ browser }) => {
  ...
  for (const id of IDS) {
    ...
    page.on('pageerror', (e) => {
      if (!NOISE.test(e.message)) errors.push('[pageerror] ' + e.message.slice(0, 220))
    })
    page.on('response', (r) => {
      if (r.status() >= 400 && !NOISE.test(r.url()))
        other.push(`http${r.status()} ${r.url().slice(0, 120)}`)
    })
    page.on('requestfailed', (r) => {
      const u = r.url()
      if (!NOISE.test(u) && !u.includes('favicon'))
        other.push(`reqfail ${r.failure()?.errorText} ${u.slice(0, 120)}`)
    })
    let ready = false
    let readyMs = -1
    let painted = -1
    const t0 = Date.now()
    try {
      await page.goto(`/demo.html?id=${id}&e2e=1`, {
  ...
  for (const r of bad)
    console.log(
      `  BAD  ${r.id}: ${!r.ready ? 'NOT-READY ' : ''}${r.errors.slice(0, 2).join(' || ')}`,
    )
