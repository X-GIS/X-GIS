...
test.describe('PMTiles cold-start at high zoom', () => {
  ...
  test('pmtiles_layered: zoom=14 cold start renders non-blank canvas', async ({ page }) => {
    ...
    await page.goto(`/demo.html?id=pmtiles_layered#14/35.68/139.76`, {
    ...
    const stats = await page.evaluate(() => {
      const canvas =
        (document.querySelector('#xgis-canvas') as HTMLCanvasElement | null) ??
    ...
    console.log(
      `[zoom14-blank] fg=${stats.fg} bg=${stats.bg} total=${stats.total} fgPct=${(stats.fgPct * 100).toFixed(2)}%`,
    )
    for (const v of stats.vtrs) {
      console.log(
        `  ${v.name}: tilesVisible=${v.tilesVisible} drawCalls=${v.drawCalls} gpu=${v.gpu}`,
      )
    }
    ...
    expect(stats.fgPct).toBeGreaterThan(0.1)
  })
})
