...
function readCanvas(
  ...
  try {
    data = ctx.getImageData(0, 0, c2.width, c2.height).data
  } catch {
    return null
  }
  const counts: number[] = []
...
function statsLine(label: string, counts: number[], painted: number, total: number): string {
  ...
  return (
    `${label}: 평균=${mean.toFixed(1)}× p95=${pct(sorted, 95)}× 최대=${sorted[sorted.length - 1]}×  >8×=${over8.toFixed(0)}%  화면덮음=${cover.toFixed(0)}%\n` +
...
export function installOverdrawCapture(map: OdMap): void {
  ...
  pre.textContent =
    '«측정»을 누르면 pitch 0° → 70° 를 자동 캡처하고 오버드로 수치(평균/p95/최대/분포)를 뽑습니다.'
  ...
  runBtn.addEventListener('click', () => {
    void (async () => {
      ...
      if (!flat && !tilt) {
        ...
        report = ''
        pre.textContent =
          '이 브라우저는 캔버스 자동 캡처가 안 됩니다.\n' +
          '아래 버튼으로 pitch 를 바꾸며 각각 직접 스크린샷 해 주세요.'
        const f0 = mkBtn('pitch 0°', '#444'),
          f70 = mkBtn('pitch 70°', '#444')
        f0.onclick = () => {
