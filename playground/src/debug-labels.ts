...
function setStatus(s: string): void {
  ...
  statusEl.style.display = s ? 'block' : 'none'
}
...
async function mount(): Promise<void> {
  ...
  const url =
    STYLES[styleParam] ??
    (styleParam.startsWith('http') ? styleParam : STYLES['openfreemap-bright']!)
  ...
  try {
    ...
    setStatus(`스타일 실패: ${(e as Error).message}`)
    return
  }
  ...
  let src: string
  try {
    src = convertMapboxStyle(styleJson as Parameters<typeof convertMapboxStyle>[0], {
      inlineGeoJSON,
    })
  } catch (e) {
    setStatus(`변환 실패: ${(e as Error).message}`)
    return
  }
  ...
  try {
    ...
    setStatus(`run() 실패: ${(e as Error).message}`)
    return
  }
...
function analyze(l: DumpLabel, icon?: DumpIcon | null): { html: string; bad: boolean } {
  ...
  if (icon) {
    ...
    if (inkGs.length) {
      ...
      if (Math.abs(gap) > tol) {
        bad = true
        out.push(
          `  <span class="bad">[텍스트-박스 어긋남 ${gap.toFixed(0)}px] text-center=${textCenter.toFixed(0)} box(${icon.name})-center=${icon.centerY.toFixed(0)}</span>`,
        )
      } else {
  ...
  for (const [oy, gs] of lines) {
    ...
    out.push(
      `  oy=${oy} "${chars}" x:${xr} rdrY:${minRy.toFixed(0)}~${maxRy.toFixed(0)}` +
  ...
  for (let i = 1; i < lineRenderY.length; i++) {
    if (lineRenderY[i]! <= lineRenderY[i - 1]! + 1) {
      bad = true
      out.push(
        `  <span class="bad">[줄겹침: line${i} rdrY≈${lineRenderY[i]!.toFixed(0)} ≤ line${i - 1} ${lineRenderY[i - 1]!.toFixed(0)}]</span>`,
      )
    }
  }
  ...
  if (bad) {
    for (const g of l.glyphs) {
      ...
      out.push(
        `    <span class="dim">${String.fromCodePoint(g.cp)} oy=${g.y.toFixed(0)} rfs=${g.rfs} bY=${g.bearingY.toFixed(0)} h=${g.height} → rdrY=${ry.toFixed(0)}</span>`,
      )
    }
  }
