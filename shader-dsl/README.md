...
| **GLSL backend**                                                                                                                                                   | **real for render pipelines** — vertex+fragment entry-IO + std140 UBO + MRT draw buffers, WebGL2 compile+render-verified (see `examples/`, #847); an SSBO lowers to a data texture by default, compute emulation is opt-in, MSAA-load fails closed |
