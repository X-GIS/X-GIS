...
| [0010](0010-read-gridded-standards-in-place.md)       | Read gridded standards in place; deprecate `.xgcov`                     | Accepted (removal landed #1279; range reader follow-up) | The data is already in a standard (S-111/S-102 are HDF5; imagery is COG; vector is PMTiles) and we own a reader for it — so read it IN PLACE via HTTP range, don't transcode it into a house blob. `.xgcov` repeats the `.xgvt` custom-format trap and is deprecated (codec + `s100ToXgcov` + `type:coverage` blob removed); the HDF5 reader becomes range-capable and the format-agnostic grid→ramp renderer survives. CORS (some NOAA S3 buckets lack it) is a proxy concern, not a format one.                                                                                                                                                                                                                                                                                                         |
| [0011](0011-coverage-overlap-catalogue-relevance.md)  | Coverage mosaic overlap — catalogue relevance, most relevant drawn last | Accepted                                                | Where two coverage domains overlap, ownership comes from the region's index in the `_coverage` Map (`regionPriority`) and nothing else; the most relevant is drawn LAST. Consolidates #1585 (arrows drew both fields) and #1602 (the drape's winner was the most recently RE-ARMED region, flipping every forecast tick). Recency is not relevance: `states` (renderer LRU) and `batches` (arrow store) both move a re-armed region to the back, and both are one property access away at every site that needs a winner. Eviction's LRU is deliberately untouched. Gate: `coverage-overlap-identity.test.ts` (readout = drape = arrows at one probe point).                                                                                                                                              |
| [0012](0012-full-mapbox-style-support-roadmap.md)     | Full Mapbox style-spec support — scope, phasing, process invariants     | Accepted                                                | "Full support" = every spec-coverage row `supported` or precisely-warned `partial`; the 15 `na` rows (sort-keys, symbol-avoid-edges, feature-state, …) are reaffirmed architecture decisions, not backlog. Four phases: A converter completeness/diagnostics (#1976-#1978), B source-level (#1489 continuation), C expression/data-driven plumbing, D renderer features (each with its own design doc). Per-item process: issue-first, fail-before corpus, byte-identity for untouched styles, full gate, gauntlet merge — live status stays in spec-coverage + gap-matrix, never duplicated here (second-ratchet lesson).                                                                                                                                                                                |
| [0013](0013-duplication-ratchet-and-consolidation.md) | Duplication ratchet + consolidation policy                              | Accepted                                                | jscpd is the detector (`.jscpd.json` = the one definition of a clone: 70 tokens / 5 lines, tests excluded); `bun run dup` is a shrink-only fingerprint baseline in the `lint` job (new clone or stale entry = red). Consolidation borrows from Linux (`lib/` + Coccinelle: helper + rewrite every copy + keep the guard), LLVM (`ADT/Support`, TableGen: generate sibling families from tables), rustc (`tidy`) and the rule of three — third copy in a package / second across a boundary extracts to the lowest importable package. Intentional twins carry a reasoned `jscpd:ignore` marker. Measured 2026-09-05: 280 clones / 3423 lines (1.47%) in 232k source lines (JS tokenizer — the TS one has a recorded blind spot); audit + work queue in `docs/plans/2026-09-05-code-duplication-audit.md`. |
