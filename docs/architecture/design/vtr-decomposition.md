...
> top-level methods). It is a _map of where the work would go_, grounded in
...
The user's quality bar: _kill god-objects by extracting **ownership**, not
code — a god class is unclear state-ownership; define who owns each piece of
state._ The prior decomposition review
...
one place (the `map.ts` Mapbox-API facade, not here).** VTR has _grown_ +310
...
_already_ been lifted out of VTR into owned collaborators with narrow
...
- `UniformRing` (`render/uniform-ring.ts`) — owns the growable ring buffer +
  ...
  via the `onGrow` callback (`:649`). The byte _layout_ stayed with VTR; only
  the _buffer lifecycle_ moved.
...
The render _passes_ were likewise lifted from `XGISMap.renderFrame` into
...
caveats, a blind ratchet _hurts_ — it would split hot loops and the
intentionally-cohesive uniform layout. The ratchet is a _consequence_ of moving
...
The class header asserts a narrow contract — _"This class manages GPU buffers,
bind groups, and draw calls only"_ (`:2-4`). The 5608-line reality is the gap
...
/ `getPendingUploadCount` (`:1025 :1030`). This cluster _allocates from_ Cluster
A's arenas and _writes into_ Cluster A's `gpuCache` — that is the one hard
seam (see §5 hazards).
...
_byte layout_ (`uniformF32[19..58]`) is written inline in `renderTileKeys` and
...
(`render/compute-layer-handle.ts`); this cluster is the _factory + lifetime
owner_ of those handles.
...
external modules; the _state_ (the per-frame cache + hysteresis) is what lives
on VTR.
...
surface; the _writes_ are interleaved in the hot loop and must stay there (§5).
...
loop. They _read across all clusters_ — this is precisely why VTR is a god
...
| `UniformRing` _(exists)_ | ring buffer + staging mirror + dirty flush                                                          | `ensure`, `resetSlot`, `allocSlot`, `stageSlot`, `flush`, `takeRetired`, `destroy`                                                                                                   | C              |
...
_coordination + hot loop_, with the rest distributed across owners that are each
...
two coupled clusters at once. The review's directive — _"hard god-objects LAST,
one at a time, render-gate + warm-harness"_ — is respected: the GPU-memory core
...
`_frameArena`; VTR forwards the three reads. _Verify:_ `_label-anchor-parity`
...
calls `stats.recordDraw(...)` instead of mutating fields inline. _Verify:_
...
contract exists. _Verify:_ `_zoom-transition-blank-tiles.spec.ts` +
...
_writes into_ tiles' `featureBindGroup` and is rebuilt on ring grow. Keep the
ring-grow rebuild as a callback the binder registers (same `onGrow` pattern as
UniformRing). _Verify:_ `_fixture-picking.spec.ts`, `_filter-gdp-z-fighting.spec.ts`,
...
The uniform _byte buffer_ (`uniformF32`/`uniformDataBuf`) does **not** move (it
...
the registry must expose `fillBgFor(layout)` cheaply. _Verify:_
...
then point `TileUploader` at its `acquireSlot`/`alloc` interface. _Verify, in
order:_ `gpu-arena.test.ts` (unchanged arena semantics) → `_dequant-parity` +
...
1. **The per-tile uniform byte pack inside `renderTileKeys` (`:4996-5174`).**
   ...
   comment at `:299-301` is explicit: _out-of-bounds typed-array writes are
   silent no-ops in JS, so a mismatch here = shader reads garbage._ This pack
   must stay **inline in the hot loop, in order**, owned by the coordinator.
   `BindGroupRegistry` owns the ring _buffer_; it does **not** own the byte
...
2. **The `renderTileKeys` per-tile loop structure + two-pass fill/stroke
   ...
   `strokeQueue` and emitted in pass 2 _after every fill has written depth_
...
4. **Per-frame draw ORDER across passes (the bucket scheduler contract).**
   ...
   dispatch; owners are invoked _within_ the existing order.
...
6. **The hoisted hot-path scratch collections (`:249-295`).** `_scratch*`
   ...
   When their owning method moves to a new class, the scratch must move _with
   it_ and stay instance-level on that class — do not reallocate per call.
...
- **Step 6 is genuinely risky and cannot be fully CI-gated.** Per ADR-0004, the
  GPU-memory/byte path's _correctness_ (pixel-match, leak behavior, globe arena
...
- **Indirection cost on the hot path.** Today `renderTileKeys` reads `this.field`
  ...
  should inline monomorphic owner calls, but this is a _measure-don't-assume_
...
- **The A↔B seam may not cleanly separate.** `TileUploader` and `GpuTileStore`
  ...
  _one_ coherent owner is still a large win over the monolith.
