...
extractions (RenderLoop, controllers) are self-described as _"RELOCATION, not a decoupling"_
...
blocking defects; verdict _"a strict L0 lint … could only ship warn-only with a large
baseline"_). The corrected sequence: **derive the charter FROM the real import graph** (a
