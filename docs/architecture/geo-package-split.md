...
- **3d — lock it.** A resident-content ratchet asserting `engine/src` has no
  ...
  ratchet green in CI. **Known 3c residue to clear here:** the engine still re-exports
  `./projection/ecef` (a thin `export * from '@xgis/shared'` shim), and `@xgis/data`'s ONLY
  remaining `@xgis/engine` import is ECEF (`tileEcefCenterFromMerc` / `lonLatToECEF`, 5 files)
  through it. Dropping the shim so those consumers import ECEF straight from `@xgis/shared` severs
  the last `data → engine` edge and leaves the engine barrel projection-free.
