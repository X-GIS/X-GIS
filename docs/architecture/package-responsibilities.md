...
| **core/backends/glsl.ts**                                                       | GLSL ES 3.00 `Backend`; std140 UBO (fed by `reflect`)                   | duplicate the walk / locally rename intrinsics; read-only SSBO lowers to a data texture by default, writes/compute/MSAA fail closed |
