...
scheduler, generic camera). deck.gl = the *layers* — domain content that authors shaders +
...
| Pipeline-state constants | BLEND_*/DEPTH_*/MSAA_STATE half of `gpu/gpu-shared.ts` |
...
style→scene. Pass *names* (`oitAccum`/`heatmapAccum`) + bucket *order* are content config
...
`PassDef[]` replaces the inline 8-step chain at `render-loop.ts:455-481`; the *order + identity
of buckets* become @xgis/map config, not engine code.
...
**Key insight:** the RHI is *more complete than "only raster routes through it" but DEAD-by-
default* — every primitive HAS a Draper, gated behind a `globalThis` flag (OFF in production):
...
1. **The fixed map-shaped pass chain.** `render-loop.ts:455-481` hardcodes the map's 8-step layer
   ...
   *dependencies* (heatmap-after-labels owns the MSAA resolve, `render-loop.ts:472`), or is a flat
...
2. **Projection generic/map split.** `camera.ts` IS the Web-Mercator camera (centerX/Y = Mercator
   ...
   *composes*, or does @xgis/map subclass it? The Mercator-metre position can't live in a generic camera.
3. **Shared vertex-format byte contracts.** `TEXT_FORMAT`/`ICON_FORMAT` are the good model, but the
   ...
   Converge all packers onto the DSL-reflected SoT *before* freezing the interface (drift = every
   poly draw corrupted).
4. **Arena / uniform-ring ownership.** Clean engine leaves, but VTR binds shared arena *sub-ranges*
