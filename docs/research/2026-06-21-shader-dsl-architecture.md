...
3. **Open/closed.** Adding a target is *one new file* implementing a contract — **zero edits to the
...
| **Intrinsic** | a known operation (`IntrinsicId`); the core owns its *semantics* (arity, signature, CPU fold), each backend owns its *spelling* | spelling: yes |
| **Pass** | an IR→IR transform; *neutral* (match→switch, const-fold) or *backend* (struct-IO flatten, sampler fuse) | backend passes: yes |
...
| **Oracle** | the CPU f64 evaluator; a *consumer* of the IR, not a render target | no (it is not a Backend) |
...
the folder name lies. The redesign separates *writer* (`backends/`), *pass* (`core/passes/`), and
*oracle* (`core/oracle.ts`), so the structure states the architecture.
...
2. **Intrinsic spelling** — the core owns the **semantics** centrally (`IntrinsicSpec`: arity,
   signature for type-check, the `cpu` fold). Each backend owns the **spelling** in *its own file*
   ...
   compile error (caught by a coverage test). *This central-semantics / per-backend-spelling split is
   the key plugin decision.*
...
5. **Passes** — `Backend.passes()` contributes backend-specific IR transforms; the driver runs
   *neutral passes → backend passes → emit*, so a backend customises the IR *before* it sees it
