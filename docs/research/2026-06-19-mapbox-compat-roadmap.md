...
- VERIFY: Layered, because CI has NO GPU (SwiftShader only compile-gates WGSL). (1) UNIT/compiler (CI-green): new compiler/src/**tests**/text-writing-mode-convert.test.ts asserts text-writing-mode:['vertical'] now emits utility label-writing-mode-vertical and lowers to LabelDef.writingMode==='vertical' (paral
...
- RISK: Snapshot regen is MANDATORY and easy to forget: **polygon-variant-snapshots**/*.wgsl pin exact WGSL bytes; the US-010 byte-equal drift gate fails until all ~8 are regenerated after the const->uniform
