...
- **I1 — offscreen tile-bake infra.** `RenderTargets`-managed RT (atlas), tile-local ortho MVP, render one vector tile's geometry into it. Verify: readback the RT shows the tile content (headless GPU readback).
