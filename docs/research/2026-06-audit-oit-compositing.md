...
*Confidence: the codebase audit (direct read) and McGuire-Bavoil/alpha-compositing primary sources are load-bearing. The "OIT is dead code, translucency is order-dependent today" conclusion is the highest-value finding and is directly evidenced by `bucket-scheduler.ts:284` + the missing routing.*
