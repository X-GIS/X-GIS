...
*Deep-research synthesis, 2026-06-08. File:line audit of the X-GIS text pipeline merged with SDF-text and map-label-placement research (the latter cross-referenced from the MapLibre source surveyed in Audit ①). Part of the 10-audit series. Claims cited inline.*
...
- **HEADLINE BUG — layout-cache hash collision returns the wrong label's glyph offsets** (`text-stage.ts:1359-1381`). The 32-bit FNV-1a `layoutCacheKey` can collide; on a false hit the cache returns label-A's `glyphOffsets` for label-B, and the **generation guard does *not* catch it** (it validates eviction-staleness, not text identity). This is the reproduced "일부만" bilingual scatter.
