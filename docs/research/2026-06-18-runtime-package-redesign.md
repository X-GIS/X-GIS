...
> shippable — the god-file split *units*, the DO-NOT-SPLIT cohesion zones, the four
...
> The half the review judged *aspirational* — the strict numbered L0–L5 "downward-only"
...
The reviewer's verdict was blunt and correct: *"Not approvable until the spine is made
consistent with the actual import graph."* The flagship principle (a numbered,
...
charter the real graph already obeys, and renumber** — *plus* it records path **(b)**
...
|---|---|---|
| B1 | `camera.ts` imports L1 (`gpu`, `gpu-shared`), L2 (`shaders/log-depth`), L4 (`loader/geojson`) — 4 upward edges vs the L0 charter; never reconciled in §3. | §2 redefines L0 (now **L0 `coords+camera`**) so the charter **permits** reading platform *constants* and pure *shader fragments*; the one genuinely cross-layer edge — `lonLatToMercator` from `loader/` — is named in §3.1 and §4 as the **single edge the lint flags**, with a 1-function fix (move the pure converter down to L0). No charter is asserted that the file violates. |
| B2 | `globe.ts:22` & `view-matrix.ts:20` import `WORLD_MERC`/`TILE_PX` from L1 `gpu-shared` — undocumented L0→L1 upward edges; migration never relocates these constants. | §2 reclassifies `WORLD_MERC`/`TILE_PX` as **L0 coordinate constants that merely *live* in `gpu-shared` today**. They are read by L0 projection math (`globe`, `view-matrix`, `camera`) and are coordinate-space facts, not platform facts. Optional refactor R-A (§5) physically moves them into `coords/` so the edge disappears; until then the lint **allowlists** these three reads explicitly. |
...
VTR 5608 / map 2956 and calls the decomposition "unexecuted"); the *enforced* sizes are the
...
> above are the ones CI actually fails on. The decomposition *units* the review affirmed are
...
> state-ownership; the fix is to define *who owns each piece of state*, give it a narrow
> interface, and let the former god class shrink to a thin coordinator that wires the owners
> and holds only the order.

LOC reduction is a *consequence* of moving ownership, applied as a "grandfather + shrink-only"
ratchet (`LOC_CEILINGS`), never the goal. A blind ≤500-line chase *hurts*: it splits hot loops
...
- **Owned state lifts out cleanly.** Inside VTR, `UniformRing` and `PrefetchScheduler` are
  *already* extracted owners with narrow interfaces, each receiving caller-side policy via a
  callback. The byte *layout* stayed with VTR; only the *buffer lifecycle* moved.
...
> **What is load-bearing here:** the *direction rule* and the *lint that enforces it* — **not**
...
The reviewed draft's spine was *L0 math → L1 gpu/platform → L2 shader → L3 render → L4
io/style → L5 facade* with "arrows downward only". That spine was **not satisfiable**:
...
`projections-table` predicates — a *downward* L1→L0 read, `gpu-shared.ts:303-309`, verified;
...
claimed style is *"consumed by render … never the reverse."* That is internally contradictory:
...
`computeSliceKey` is *defined* at `data/eval/filter-eval.ts:186`; its other consumers
...
resolution render consumes (`color-ramp`, paint-shape resolution) is likewise an L2/L3 *leaf
below or beside render*, never a layer above it.
...
| `:5` | `WORLD_MERC, TILE_PX` from `../gpu/gpu-shared` | L0 constants (housed in L1) | **Legal-by-reclassification.** Coordinate constants are L0 facts; they merely *live* in gpu-shared today. R-A moves them to `coords/`; until then allowlisted. Same for `globe.ts:22`, `view-matrix.ts:20`. |
...
So the corrected charter is satisfied by all of L0 *except* two named, allowlisted reads
...
lint (C3):** it enforces every edge the graph already satisfies and carries a *small, named*
...
> The reviewer: *"the decomposition realism problem is NOT in the splits — it is in the
> TAXONOMY layering."* The taxonomy is fixed above. The split units below — low-blast
...
- **Step B — the class becomes a thin coordinator.** Keeps external refs, per-frame *order*,
...
1. **Hot loops.** VTR's per-tile `renderTileKeys` loop and TextStage's `prepare()` shaping loop
   read across *all* clusters by design — they *become* the thin coordinator. Owners are
   *received as collaborators*; the loop body stays one method.
...
3. **Draw order.** Opaque → OIT → translucent-strokes → points → labels is fixed for alpha
   ...
   owners *within* the order; never reorders.
...
(corrected charter §2.2) — *not* a violation. The only edge the lint flags here is the
...
(`configureFromRenderer(renderer)` on VTR), *then* `SceneBuilder` lifts cleanly.
...
cycle exists *only* because `render/paint-shape-resolve.ts:25` imports them via `'./renderer'`
...
bind-group-drift invariant test that reads the static layout entries *without a GPUDevice*.

**Shared-state hazard (kept):** bind-group co-ownership — `rebuildUniformBindGroups` +
`setPaletteColorAtlas` + `setSpriteAtlas` reconstruct the *same* immutable `GPUBindGroup` from
...
`unproject.ts`): it is the single dependency shared by *both* future owners and is
...
`_cache*` shadows compare *values* (do **not** introduce a dirty-flag protocol);
...
The order optimizes **risk × value × coupling**: (a) prove the pattern on a *clean-boundary,
CI-gated* unit before the hot path; (b) provider before consumer; (c) highest-value-highest-risk
...
2. **Land the boundary lint, enforcing-with-named-allowlist** (§2.3 / C3): it asserts the four
   ...
   **explicit allowlist entry with a TODO pointing at R-A/R-B/R-C (§5)**. This is the *honest*
...
strongly CI-covered. Lands *before* VTR/map.ts because the render-loop frame-push targets its
`CameraState` record.

**Wave 2 — TextStage (independent standalone pilot; build the missing test seam, L3).** Zero
coupling to other god-objects. Begins by *writing characterization tests* for `prepare()` before
...
placed *after* the pattern is proven and the provider seam de-risked. They share the
...
extraction depends on VTR first absorbing `configureFromRenderer(renderer)`. The *cleanest* cut
...
only if the edges still matter. These are *real refactors*, explicitly out-of-band from the
relocate-only waves.
...
- **R-B — drop the pure converter to L0.** Move `lonLatToMercator` from `loader/geojson` into
  ...
  back-compat. Retires the `camera.ts:3` violation — the *one true* upward edge.
...
> **Why these are fenced as optional:** per C2/C3, the load-bearing artifact is the *enforced
> direction rule*, not a clean directory. The lint already enforces direction with a small named
> allowlist; R-A/R-B/R-C merely shrink the allowlist to empty. Doing them is good hygiene;
> *requiring* them before any split would be the same "rename-only move that is secretly a
...
- **Tier 2 — local / pre-push, real GPU (headed).** pixel-match survey,
  `_projection-label-onscreen` + `_label-anchor-parity` (label *position*, not pixels), globe /
  non-Mercator render matrices, the autonomous screenshot-eyeball loop. Any cut touching the
  raster path (the GPU-memory triad) ships *alone* with a warm-harness numeric perf comparison
...
> barrel. The decomposition adds *internal* modules only; the consumer API surface is unchanged.
...
- **Open-Q1 — `getMaxDpr` injection (R-C).** Is there any frame where the camera needs DPR
  *before* L4 can supply it? If yes, R-C needs a default; if no, R-C is trivial.
