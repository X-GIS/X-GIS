...
class Capabilities {
  ...
  has(c: Capability) { return this.set.has(c) }
  /** Does this backend cover everything a module requires? */
  covers(reqs: ReadonlySet<Capability>) { return [...reqs].every((c) => this.set.has(c)) }
  missing(reqs: ReadonlySet<Capability>) { return [...reqs].filter((c) => !this.set.has(c)) }
}
...
class Node {
  ...
  add(o: Node | number) { return this.bin('+', o) }
  mul(o: Node | number) { return this.bin('*', o) }
  private comp(f: string) {
    const elem: ShaderType = this.type.kind === 'vec' ? { kind: 'scalar', scalar: this.type.elem } : F32
...
class Builder {
  ...
  store(slot: string, value: Node) { this.stmts.push({ s: 'store', target: slot, value: value.expr }) }
  ret(value: Node) { this.stmts.push({ s: 'ret', value: value.expr }) }
}
function fn(name: string, params: Record<string, ShaderType>, ret: ShaderType,
  build: (b: Builder, p: Record<string, Node>) => void): FuncDecl {
  const ps = Object.entries(params).map(([n, type]) => ({ name: n, type }))
  const args = Object.fromEntries(ps.map((p) => [p.name, new Node({ op: 'ref', type: p.type, name: p.name })]))
  const b = new Builder(); build(b, args); return { name, params: ps, ret, body: b.stmts }
}
function entry(name: string, stage: Stage, io: { inputs: IOSlot[]; outputs: IOSlot[] },
  build: (b: Builder, inp: Record<string, Node>) => void): EntryDecl {
  const inp = Object.fromEntries(io.inputs.map((s) => [s.name, new Node({ op: 'ref', type: s.type, name: s.name })]))
  const b = new Builder(); build(b, inp); return { name, stage, inputs: io.inputs, outputs: io.outputs, body: b.stmts }
}
const builtin = (which: IOBuiltin, name: string, type: ShaderType): IOSlot => ({ name, type, bind: { kind: 'builtin', which } })
const location = (index: number, name: string, type: ShaderType, o?: { flat?: boolean }): IOSlot =>
  ({ name, type, bind: { kind: 'location', index }, interp: o?.flat ? 'flat' : undefined })
...
const ew = (xs: (number | Vec)[], f: (...n: number[]) => number): number | Vec =>
  xs.every((x) => typeof x === 'number') ? f(...(xs as number[]))
    : (xs.map((x) => Array.isArray(x) ? x : [x]) as Vec[]).reduce((acc, _v, _i, arr) =>
      (arr[0] as Vec).map((_, j) => f(...arr.map((v) => v[j] ?? v[0]))), [] as Vec)
const INTRINSICS: Record<IntrinsicId, IntrinsicDef> = {
  max: { wgsl: (a) => `max(${a[0]}, ${a[1]})`, glsl: (a) => `max(${a[0]}, ${a[1]})`, cpu: (a, b) => ew([a, b], Math.max) },
  log2: { wgsl: (a) => `log2(${a[0]})`, glsl: (a) => `log2(${a[0]})`, cpu: (a) => ew([a], Math.log2) },
  pow: { wgsl: (a) => `pow(${a[0]}, ${a[1]})`, glsl: (a) => `pow(${a[0]}, ${a[1]})`, cpu: (a, b) => ew([a, b], Math.pow) },
  // DIVERGENT: WGSL select(falseVal, trueVal, cond); GLSL ES has no select() → ternary.
  select: { wgsl: (a) => `select(${a[0]}, ${a[1]}, ${a[2]})`, glsl: (a) => `(${a[2]} != 0.0 ? ${a[1]} : ${a[0]})`,
    cpu: (f, t, c) => (c ? t : f) },
}
...
const WGSL: Backend = {
  id: 'wgsl', caps: new Capabilities(new Set(['compute', 'storageBuffer', 'msaaTextureLoad'])),
  typeName: (t) => t.kind === 'scalar' ? t.scalar : t.kind === 'vec' ? `vec${t.n}<${t.elem}>` : 'void',
  literal: (v, t) => t.kind === 'scalar' && t.scalar === 'f32' ? f32Lit(v) : t.kind === 'scalar' && t.scalar === 'u32' ? `${v}u` : `${v}`,
  intrinsic: (id, a) => INTRINSICS[id].wgsl(a),
  localDecl: (n, _t, init) => `let ${n} = ${init};`,
  lowerEntry(e) {
    const at = (s: IOSlot) => s.bind.kind === 'builtin'
      ? `@builtin(${s.bind.which})` : `${s.interp === 'flat' ? '@interpolate(flat) ' : ''}@location(${s.bind.index})`
    const params = e.inputs.map((s) => `${at(s)} ${s.name}: ${WGSL.typeName(s.type)}`).join(', ')
    const outFields = e.outputs.map((s) => `  ${at(s)} ${s.name}: ${WGSL.typeName(s.type)},`).join('\n')
    const stores = e.body.filter((x): x is Extract<Stmt, { s: 'store' }> => x.s === 'store')
      .map((x) => `  out.${x.target} = ${emitExpr(x.value, WGSL)};`).join('\n')
    const locals = e.body.filter((x): x is Extract<Stmt, { s: 'local' }> => x.s === 'local')
      .map((x) => `  ${WGSL.localDecl(x.name, x.type, emitExpr(x.value, WGSL))}`).join('\n')
    return `struct ${cap(e.name)}Out {\n${outFields}\n}\n@${e.stage} fn ${e.name}(${params}) -> ${cap(e.name)}Out {\n  var out: ${cap(e.name)}Out;\n${locals}\n${stores}\n  return out;\n}`
  },
  preamble: () => '',
}
const GLSL_ES300: Backend = {
  id: 'glsl-es300', caps: new Capabilities(new Set()),
  typeName: (t) => t.kind === 'scalar' ? ({ f32: 'float', u32: 'uint', bool: 'bool' }[t.scalar])
    : t.kind === 'vec' ? (t.elem === 'f32' ? `vec${t.n}` : `uvec${t.n}`) : 'void',
  ...
  lowerEntry(e) {
    ...
    const ins = e.inputs.map((s) => `layout(location=${s.bind.kind === 'location' ? s.bind.index : 0}) in ${GLSL_ES300.typeName(s.type)} ${s.name};`)
    const outs = e.outputs.filter((s) => s.bind.kind === 'location')
      .map((s) => `${s.interp === 'flat' ? 'flat ' : ''}out ${GLSL_ES300.typeName(s.type)} v_${s.name};`)
    const body = e.body.map((x) => x.s === 'local' ? `  ${GLSL_ES300.localDecl(x.name, x.type, emitExpr(x.value, GLSL_ES300))}`
      : x.s === 'store'
...
const storeTargetGlsl = (e: EntryDecl, slot: string) => {
  ...
  return o.bind.kind === 'builtin' ? ({ position: 'gl_Position', frag_depth: 'gl_FragDepth', vertex_index: '' }[o.bind.which]) : `v_${slot}`
}
...
function evalCpu(f: FuncDecl, args: Record<string, number | Vec>): number | Vec {
  ...
  const ev = (e: Expr): number | Vec => {
    switch (e.op) {
      ...
      case 'ref': return env[e.name]
      case 'binop': { const a = ev(e.a) as number, b = ev(e.b) as number; return e.bop === '+' ? a + b : a * b }
      case 'member': { const v = ev(e.base) as Vec; return v[{ x: 0, y: 1, z: 2, w: 3 }[e.field as 'x']] }
      case 'construct': return e.args.map((x) => ev(x) as number)
      case 'intrinsic': return INTRINSICS[e.id].cpu(...e.args.map(ev))
    }
  }
...
function emitExpr(e: Expr, be: Backend): string {
  switch (e.op) {
    ...
    case 'binop': return `(${emitExpr(e.a, be)} ${e.bop} ${emitExpr(e.b, be)})`
    case 'member': return `${emitExpr(e.base, be)}.${e.field}`
    case 'construct': return `${be.typeName(e.type)}(${e.args.map((x) => emitExpr(x, be)).join(', ')})`
    case 'intrinsic': return be.intrinsic(e.id, e.args.map((x) => emitExpr(x, be)))
  }
}
function emitFunc(f: FuncDecl, be: Backend): string {
  const params = f.params.map((p) => be.id === 'wgsl' ? `${p.name}: ${be.typeName(p.type)}` : `${be.typeName(p.type)} ${p.name}`).join(', ')
  const sig = be.id === 'wgsl' ? `fn ${f.name}(${params}) -> ${be.typeName(f.ret)}` : `${be.typeName(f.ret)} ${f.name}(${params})`
  const body = f.body.map((s) => s.s === 'local'
    ? `  ${be.localDecl(s.name, s.type, emitExpr(s.value, be))}` : `  return ${emitExpr(s.value, be)};`).join('\n')
  return `${sig} {\n${body}\n}`
}
function compile(m: Module, be: Backend): { backend: string; code: string } {
  const missing = be.caps.missing(m.requires)
  if (missing.length) throw new UnsupportedFeatureError(
    `this shader requires { ${missing.join(', ')} } not supported by backend '${be.id}'.`)
  const parts = [...(m.funcs ?? []).map((f) => emitFunc(f, be)), ...(m.entries ?? []).map((e) => be.lowerEntry(e))]
...
const applyLogDepth = fn('apply_log_depth', { pos: Vec4f, fc: F32 }, Vec4f, (b, { pos, fc }) => {
  const z = b.let_('z', log2(max(f32(1e-6), pos.w.add(1))).mul(fc).mul(pos.w))
...
hr('one source → WGSL'); console.log(compile(mathMod, WGSL).code)
hr('one source → GLSL ES 300'); console.log(compile(mathMod, GLSL_ES300).code)
hr('structured IO → WGSL'); console.log(compile(quadMod, WGSL).code)
hr('structured IO → GLSL ES 300'); console.log(compile(quadMod, GLSL_ES300).code)
...
try { compile(continentMatch, GLSL_ES300) } catch (e) { console.log('  ' + (e as Error).constructor.name + ': ' + (e as Error).message) }
console.log(`  WGSL covers it? ${WGSL.caps.covers(continentMatch.requires)} · GLSL covers it? ${GLSL_ES300.caps.covers(continentMatch.requires)}`)
