...
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `typeshade.dev`, `typeshade.com` | REGISTERED by the owner 2026-09-07 07:30 UTC via Gabia (expire 2027-09-07). **Cloudflare zones created 2026-09-07 ~07:52 UTC** (see B.4); status `pending` until the registrar delegates | at Gabia: nameservers → `gwen.ns.cloudflare.com`, `scott.ns.cloudflare.com` for both; then the DS records in B.4 if Gabia accepts DS entry; auto-renew + 2FA on   |
...
| GitHub `typeshade`               | **organization CREATED 2026-09-07** (verified: Organization, 0 public repos, 0 public members)                                                                                           | require 2FA for members; base permission read; set website `typeshade.dev` and a one-line description; the mirror and adapter repositories go under it (§1, §6.2) |
...
### B.2b Website deployment (2026-09-07 23:12 UTC)

| Asset                           | State                                                                                                                                                                                                                                                                                                                                                                          | Owner action                                                                                     |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| `typeshade/typeshade.github.io` | **LIVE.** Site v2 (Astro 6 + Tailwind 4; `vendor/shader-dsl` = the mirror pinned at `29c9614`), main `eeed14c`; built and deployed by its own `deploy.yml` (push to main → `configure-pages` enablement → `deploy-pages`). Design record in the repo's `docs/design/00–07`. Carried from the X-GIS-bound session by a one-off Actions job on a temporary branch, since removed | keep `TYPESHADE_GITHUB_IO` only if another transport is ever needed; otherwise delete the secret |
| `typeshade.dev`                 | **RESOLVING.** Cloudflare DNS (DNS only): A `185.199.108–111.153`, AAAA `2606:50c0:8000–8003::153`, `www` CNAME `typeshade.github.io`; custom domain bound on Pages (`typeshade.github.io` → 301 → `typeshade.dev`); `https://typeshade.dev/`, `/og.png`, `/llms.txt` return 200; the live `index.html` and `llms.txt` are byte-identical to the locally verified build        | Settings → Pages → **Enforce HTTPS** (plain `http://` still answers 200 at the time of writing)  |
