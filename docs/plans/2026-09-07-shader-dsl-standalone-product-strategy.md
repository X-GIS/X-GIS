...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TypeShade is an independent project**; `typeshade/typeshade` is the development home (issues, pull requests, CI, releases) and X-GIS a consumer (owner decision 2026-09-07; reverses the monorepo-first half of #1681) | ADR-0014, #2660                           | The public face names no consumer — no TypeShade surface names or links X-GIS (the MapLibre ↔ deck.gl shape, not the Symfony one). Phase 1 (#2660) is the mirror gaining its own CI + a Tint/WebGL2 compile gate and X-GIS-free public docs, authored under `shader-dsl/` and carried by the existing subtree split; the site then re-sources its proof rungs to that CI. **The move itself — X-GIS consuming the repository, the mirror workflow retiring, the docs site leaving `x-gis.github.io`, the rename at `0.1.0` — is a separate intent**, not this document |
...
| A separate source repository for the DSL           | Was rejected in #1681; **REVERSED 2026-09-07 (ADR-0014)** — the repository is now the development home                   |
...
5. **Repository topology** — DECIDED 2026-09-07: **ADR-0014**. `typeshade/typeshade` is the
   development home, not a read-only mirror of a monorepo subdirectory; issues and pull
   requests move there once the move completes, and X-GIS becomes a consumer. Phase 1
   (#2660) is the mirror's own CI and X-GIS-free docs; the move is a separate intent. The
   adapters are separate repositories either way.
...
## Appendix A — Name screen (2026-09-07)

A knock-out screen, not a clearance opinion. Each instrument was validated against a
known positive before a "clean" result was believed (CLAUDE.md §12: a zero from an
unvalidated instrument is a broken ruler, not a finding).

| Instrument                        | Validated with                                              | Covers                                                                                   |
| --------------------------------- | ----------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| Web knock-out search              | —                                                           | products, companies, apps using the word                                                 |
| Trademarkia search page           | `tailwind` → 121 marks                                      | **USPTO only**; page 1; live and dead marks                                              |
| npm / PyPI / crates.io registries | `tailwindcss`, `tailwind`, `serde` → 200                    | package names                                                                            |
| RDAP via rdap.org                 | `tailwindcss.com`, `.dev`, `google.app` → 200; random → 404 | `.com` `.dev` `.app` (**`.io` unreliable**: `google.io` → 404, so `.io` is not reported) |
| GitHub profile page               | existing accounts render; `corusca` → 404                   | user/org name                                                                            |

**Not screened, by instrument failure:** EUIPO / TMview (API returned 405), WIPO Global
Brand Database (captcha-gated SPA), KIPRIS (SPA; no public GET). **EU and KR registers
must be searched by hand** before any filing — KIPRIS with the Latin spelling AND both
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `typeshade.dev`, `typeshade.com` | REGISTERED by the owner 2026-09-07 07:30 UTC via Gabia (expire 2027-09-07). **Cloudflare zones created 2026-09-07 ~07:52 UTC** (see B.4); status `pending` until the registrar delegates. **`typeshade.dev` LIVE on GitHub Pages since 2026-09-07 23:05 UTC (B.5)** | at Gabia: nameservers → `gwen.ns.cloudflare.com`, `scott.ns.cloudflare.com` for both; then the DS records in B.4 if Gabia accepts DS entry; auto-renew + 2FA on   |
| `typeshade.app`                  | unregistered; skipped by decision (a `play.typeshade.dev` subdomain covers the playground)                                                                                                                                                                          | none                                                                                                                                                              |
| `typeshade.io`                   | unknown (rdap.org unreliable for `.io`)                                                                                                                                                                                                                             | check at the registrar                                                                                                                                            |
| npm `typeshade`                  | **PUBLISHED** `typeshade@0.0.0` 2026-09-07 07:43 UTC (placeholder; README points at typeshade.dev and the monorepo; maintainer su.noh)                                                                                                                              | ship real releases behind it; the one-day publish token used was revoked by the owner after the publish                                                           |
| npm scope `@typeshade`           | **PUBLISHED** `@typeshade/core@0.0.0` 2026-09-07 07:43 UTC — the scope now carries a package, so the org and scope are confirmed                                                                                                                                    | 2FA stays on; CI publishes with a granular token later                                                                                                            |
| PyPI · crates.io `typeshade`     | free                                                                                                                                                                                                                                                                | optional placeholders only if a Python / Rust artifact is ever planned                                                                                            |
| GitHub `typeshade`               | **organization CREATED 2026-09-07**; **3 public repositories**: `typeshade` (the mirror, live since 08:34 UTC), `typeshade.github.io` (the site, Pages), `.github` (empty)                                                                                          | require 2FA for members; base permission read; set website `typeshade.dev` and a one-line description; the mirror and adapter repositories go under it (§1, §6.2) |
...
### B.4 Cloudflare setup (2026-09-07, by API)

Done with a three-hour zone-scoped token minted from the owner's account token and deleted
afterwards. Both zones sit on the Free plan and read `pending` until Gabia delegates.

| Item                          | typeshade.dev                                                                                      | typeshade.com                                                                                                                                                                    |
| ----------------------------- | -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Zone id                       | `4973bd1b5758a62df53fb84c616986fb`                                                                 | `bebef00934edd23d2b844a2293cb152c`                                                                                                                                               |
| Nameservers to set at Gabia   | `gwen.ns.cloudflare.com`, `scott.ns.cloudflare.com`                                                | same                                                                                                                                                                             |
| Parked-mail records           | TXT `@` `v=spf1 -all`; TXT `_dmarc` `v=DMARC1; p=reject; sp=reject; adkim=s; aspf=s`; MX `@` `.` 0 | same                                                                                                                                                                             |
| Redirect                      | —                                                                                                  | AAAA `@` `100::` (proxied), CNAME `www` (proxied); Redirect Rule `d865f442…`: host `typeshade.com` or `www.typeshade.com` → `https://typeshade.dev` + path, 301, query preserved |
...
Still owner-side: the DS records if Gabia offers DNSSEC entry for the TLD. The A records and
the Pages site: done, see B.5.

### B.5 typeshade.dev — first deploy and live verification (2026-09-07, 23:05 UTC)

The site repository is `typeshade/typeshade.github.io` (`main` = `eeed14c`, `vendor/shader-dsl`
pinned to mirror commit `29c9614`). Its `deploy.yml` (push to `main` → build → `qa:links` →
`configure-pages` with `enablement: true` → `deploy-pages`) ran once, run 34168771665, and every
step was green: `bun run build` 6 s, `qa:links` **21 destinations resolved, 0 warn, 0 broken**,
Pages enabled from the workflow, deploy finished 23:05:21 UTC. The custom domain is set (the
artifact carries `CNAME` = `typeshade.dev`, and GitHub serves the domain), so the last open
item of B.4 — "the `A` records, DNS only, with Enforce HTTPS" — is done except for the HTTPS
toggle below.

**DNS (Cloudflare, DNS only, resolved from 1.1.1.1):** `typeshade.dev` A `185.199.108.153`
`.109.153` `.110.153` `.111.153`; AAAA `2606:50c0:8000::153` `8001::153` `8002::153`
`8003::153`; `www` CNAME `typeshade.github.io`. The parked-mail records of B.4 are unchanged.

| Request                            | Response (measured from a container, through its egress proxy)                                                                                          |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `https://typeshade.dev/`           | **200**, HTTP/2, `server: GitHub.com`, `last-modified` 23:05:16 UTC, 62 209 B; certificate `CN=typeshade.dev`, issuer Let's Encrypt, expires 2026-12-06 |
| `https://www.typeshade.dev/`       | 301 → `https://typeshade.dev/`                                                                                                                          |
| `https://typeshade.com/`           | 301 → `https://typeshade.dev/` (the B.4 Cloudflare rule)                                                                                                |
| `https://typeshade.github.io/`     | 301 → `http://typeshade.dev/` — plain `http`, which is GitHub's redirect while Enforce HTTPS is off                                                     |
| `http://typeshade.dev/`            | **200 — not a redirect.** Enforce HTTPS is OFF on the Pages site (owner action below)                                                                   |
| `/llms.txt` · `/sitemap-index.xml` | 200 · 200                                                                                                                                               |
| `/does-not-exist`                  | 404, the site's own `404.html`                                                                                                                          |

**The served bytes are the build.** All 22 files of `dist/` from a local `bun run build` of
`eeed14c` (submodule `29c9614`) are SHA-256-identical to the files fetched from
`https://typeshade.dev/` — `index.html`, `404.html`, the four `_astro/` assets, both woff2, the
four hashed artifacts and their `.sha256` sidecars, `llms.txt`, `robots.txt`, `CNAME`, both
sitemaps. So the QA gates below, run against a local copy of those live bytes, are a verdict on
what is served, not on a rebuild that might differ.

**Live QA — the site repository's `scripts/qa/*`, design §10 gates, run 2026-09-07 23:17–23:22
UTC against the byte-verified mirror of the live site on `headless_shell` 1194 (SwiftShader
WebGPU + WebGL2, `chromium_headless_shell-1194`, both backends).**

| Gate     | Result                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| G1       | `bun run build` **6.02 s** median of three (6.02 / 6.02 / 5.78; budget 8 s); CI's own build step 6 s                                                                                                                                                                                                                                                                                                         |
| G2       | CLS **0.00140 / 0.00141** (fonts normal / woff2 blocked), h1 **151.1875 px in both** — identical to the values design §3 records after the `Inter Fallback` fix                                                                                                                                                                                                                                              |
| G3       | INV-1 and INV-2 **pass** at 1440 and 1024, in both font states (hero text right edges 708 / 500 ≤ threshold; CTA row ≤ `--scrim-p0` 732 / 524)                                                                                                                                                                                                                                                               |
| G4       | Contrast on **both backends**, 25 frames each — the sample floor, every element `stoppedRisingLast4s` true (WebGPU 122 s at 5.1 s/frame, WebGL2 49 s at 2.0 s/frame): h1 **14.1:1** (floor 7), sub **9.34 / 9.38**, status **9.20**, caption **7.11** (floor 4.5), CTA ghost border **6.38 / 6.72** (floor 3), nav wordmark **18.33**, nav mark stroke **18.33**, nav links **12.18** — **all pass on both** |
| G5       | Clear-zone maximum luminance above L 0.05 on **100 % of frames on both backends** (minimum 0.6017, final 0.733) — **pass**                                                                                                                                                                                                                                                                                   |
| G6       | `scrollWidth === clientWidth` at 320 / 390 / 768 / 1440 and at 200 % text zoom — **pass**                                                                                                                                                                                                                                                                                                                    |
| G7       | axe **0 violations** at 1440 and 390 (the `color-contrast` rule reports `incomplete` over the canvas — G4 is that measurement); arms 3 (reduced motion) and 4 (no GPU API) and, via the mount census, arms 1 (WebGPU) and 2 (`?forcegl2=1`): **0 console errors, 0 page errors, 0 HTTP ≥ 400**; rAF loops **1** normal / **0** reduced                                                                       |
| G8       | **5 mounts, 1 animating**, 3 distinct `data-example` = 3 JSON script tags, `frames === 1` on the four still mounts, the `forceWebGl2` mount reports `webgl2`; on `?forcegl2=1` every mount reports `webgl2` and the captions say so; on the no-GPU arm all five report `none` with the build-time caption                                                                                                    |
| G9 / G11 | every row **PASS** — forbidden words, token checks, R-6/R-7, R-12/13/14 (`llms.txt` numerals all present, pinned commit `29c9614`), honesty sweep, host names 3 = 3 inside `SOURCES`, R-15 injective on accessible names over 42 links (`/`) + 13 (`/404.html`)                                                                                                                                              |
| G10      | banded → smooth discontinuity at **50 % vs `--b4-split` 50 %**, 16 vs 168 distinct levels (10.5×), **both backends** — the value design §10 records                                                                                                                                                                                                                                                          |
| G12      | tripwire **red as documented** (design §10 note 3): CTA 0.1727 · h1 0.0458 · canvas patch 0.0450 · caption 0.0135 — the caption rung cannot be reached by its lever; the human 5-second run decides                                                                                                                                                                                                          |

**Still owner-side after this pass.** (1) **Enforce HTTPS** on the Pages site (Settings → Pages;
the certificate is issued, so the box is now enabled) — until then `http://typeshade.dev/` serves
the page in clear and `typeshade.github.io` redirects to `http://`. (2) The B.3 items unchanged:
DS at Gabia, KIPRIS, the attorney search; org hygiene (2FA, website, description). (3) Design §10
note 4's manual high-contrast spot-check and the five-stranger test — neither is a headless
measurement. (4) The mirror has moved past the site's pin (`typeshade/typeshade` `main` =
`7d675f7`, the site pins `29c9614`); moving the pin is a deliberate commit per the site README,
not a drift to fix — **Phase 1 (#2660) is what moves it**, since the site's proof rungs re-source
