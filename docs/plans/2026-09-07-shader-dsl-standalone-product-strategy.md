...
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `typeshade.dev`, `typeshade.com` | **REGISTERED by the owner 2026-09-07 07:30 UTC** via Gabia, Inc. (RDAP; expire 2027-09-07; DNSSEC off; `.dev` in its add-grace period)  | auto-renew + registrar 2FA on; `.com` → 301 to `.dev`; parked-mail records (SPF `v=spf1 -all`, DMARC `p=reject`, null MX); DNSSEC when the registrar supports it  |
...
| npm `typeshade`                  | still free — nothing published under `typeshade` or `@typeshade/*` (registry 404, checked 07:5x UTC)                                    | publish the `0.0.0` placeholder with a README pointing at the project, then ship real releases behind it                                                          |
| npm scope `@typeshade`           | org `typeshade` **created by the owner 2026-09-07** (owner-reported; npmjs.com refuses this environment, so not independently verified) | 2FA on the account; publish `@typeshade/core@0.0.0` so the scope carries a package                                                                                |
...
1. ~~Domains~~ — DONE 2026-09-07: `.dev` and `.com` registered (`.app` skipped). WHOIS privacy, auto-renew and 2FA to confirm at the registrar.
2. ~~GitHub org `typeshade`~~ — DONE 2026-09-07 (verified). The mirror repository (§1) and the adapter repositories (§6.2) go under it.
3. npm org `typeshade` — DONE 2026-09-07 (owner-reported). Still open: placeholder `typeshade@0.0.0` and `@typeshade/core@0.0.0`; 2FA on the account.
