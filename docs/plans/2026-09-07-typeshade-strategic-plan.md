...
| **D14** | Repository topology and public surface — the mirror as a read-only derivative of the monorepo (#1681, PS §1) versus the repository as the development home                                                                                                       | (a) keep #1681's model and de-brand the page; (b) **independent project** — `typeshade/typeshade` is the development home (issues, PRs, CI, releases), X-GIS a consumer, and no public TypeShade surface names or links X-GIS                                                                       | **(b) — DECIDED by the owner 2026-09-07; ADR-0014.** (a) fails on its own terms: the proof links resolve into `X-GIS/X-GIS` because that is where the gates run, so de-branding hides the dependency rather than removing it. Phase 1 is #2660 (the mirror's own CI + a Tint/WebGL2 compile gate + X-GIS-free docs, carried by the existing subtree split); the move itself is a separate intent | decided 2026-09-07; Phase 1 before the site copy re-sources | #2660; the site's proof rungs (§14); D6's consumption model; the rename at `0.1.0`    |
