...
machinery that keeps the boxes and arrows *true* two years later.
...
root that may import everything; `rhi-webgpu` is the *only* package allowed to see native
...
The scanner has three assertions, and the interesting one is the first: two *known* edges
...
- **Strict-equality baselines** (fail when the count *changes in either direction*) for
...
*liveness proofs*: the gate's own regex must find a planted positive and ignore a planted
...
offsets; a pass-order array from which the render chain is built *constructively*
...
supersede, never rewrite), and *the rejected alternative is written down with the
measurement that rejected it*. A few worth reading in full:
...
- **Read standards in place (0010)**: the project shipped a bespoke binary container for
  scientific data — twice — before writing down the rule: *a reader for a standard is
  legitimate; transcoding a standard into a house blob is not.* You lose the ecosystem and
...
- **One producer per uniform block (0009)**: after a field was wired into three of six
  ...
  *write cadence* so writer-completeness drift becomes unrepresentable, then delete the
...
declined: the cross-pass couplings are a small fixed set, so they're *declared as
metadata* on a flat list instead — which pass owns which clear, whose depth store depends
...
4. Prefer making drift *unrepresentable* (constructive building, cadence-split producers,
