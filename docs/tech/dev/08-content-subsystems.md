...
whose _key_ had to become an integer hash — the string key was the top GC source at 5,000
...
writing composed _directly_ — offsets march down at **em pitch** (PBF has no vertical
...
81° that _stable_ identity was deciding a **depth** question — "Seoul" < "Shanghai" let
the far city occlude the near one. _Stable is not the same as correct._ The fix inserts a
...
none), identity at pitch 0 _by construction_, and correct at the screen edge where the
...
micro-lesson: `hover:opacity-100` once compiled cleanly into _nothing_. Unhandled
...
_through_ subexpressions (an expression is only as GPU-safe as its parts — the
...
`na` rows are _reaffirmed architecture decisions, not backlog_.
...
a house blob is not** — you lose the entire ecosystem _and_ HTTP-range streamability.
...
water" independently, and what they were all actually using was _recency_, which
...
2. **The structural insight** (after a "drifting arrows" attempt blinked): moving a
   ...
   _a fade on a large recognizable object IS a blink_. Particle systems get away with it
   ...
   **components, never speed+direction** (interpolating headings across 0°/360° averages
   to _backwards_), advecting anisotropically (`cos φ`), and clearing only on fresh
   targets (a recursive filter doesn't overwrite stale texels — it feeds on them).
3. **The synthesis** for chart arrows: the S-111 portrayal catalog binds a _function_ of
   ...
   _displacement_ (origins keep CPU-packed double precision; f32 suffices for the offset),
   then goes fully stateless: **a phase holds no position**, so instance count becomes a
   per-frame decision, fades become possible, and frame zero _is_ the standard's static
   ...
   composition the forward path already owns _will_ drift).
...
multiplies the Earth radius, so trig error lands in _meters_ (1.17 km measured on a
software rasterizer) — and feeding a more precise _input_ measurably didn't help; the fix
...
External-renderer interop is, deliberately, a _documented set of contracts_ rather than a
...
- Read standards in place, but _measure_ the range-vs-whole crossover; discover through
