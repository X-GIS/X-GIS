...
per frame at *idle*; a cache whose "256 entries" is 64 MB on one dataset and 4 GB on
...
All GPU access goes through an RHI: opaque handles, buffers declared by *semantic role*
...
`engine` package can't even *name* a native GPU type — its tsconfig has `types: []`, so
neutrality is compiler-enforced). WebGL2's gaps are bridged where honest — storage buffers
become 2D data textures with the element type on the *descriptor* (an internal-format
...
a fail-loud stub, and the fallback chain is *data* — an ordered provider array that renews
...
zoom. And after render bundles removed command-encoding cost, the *remaining* per-layer
...
bytes into the *old* buffer first (already-recorded draws must not read stale data),
...
witness measured "zero pixels moved" *on the shipping path*: a live assertion faithfully
...
The arena's contract radiates outward: everything downstream receives *windowed subarray
views* — `byteOffset ≠ 0` by construction. Which sets up the project's best-known testing
...
*different renderer's bytes* out of the shared arena. No error anywhere; the draw simply
...
gates of that path's *consumers*, not just the tests of the thing you touched.

## Geometry residency: three buffer layers

Big immutable geometry (polygon vertices/indices) lives in a few large **GPU arenas** —
a linear allocator with free lists keyed by *exact* size (bump allocation makes size-class
...
reset-only-when-empty reclaim, and compaction that ping-pongs into a *fresh* buffer during
...
coverage regions (64 MB LRU where draw order is *decoupled* from LRU order — recency is
...
high-water mark, drain to 60 %) and — subtle — reads *different quantities* for the two
