...
async function main() {
  ...
  const ab = headerBytes.buffer.slice(
  ...
  console.log(
    `  bounds:              [lon ${header.minLon}..${header.maxLon}, lat ${header.minLat}..${header.maxLat}]`,
  )
  console.log(
    `  bounds (size):       ~${((header.maxLon - header.minLon) * 111 * Math.cos((43.77 * Math.PI) / 180)).toFixed(1)}km × ~${((header.maxLat - header.minLat) * 111).toFixed(1)}km`,
  )
  console.log(
    `  center:              z=${header.centerZoom} at (${header.centerLat}, ${header.centerLon})`,
  )
  console.log(`  numTileEntries:      ${header.numTileEntries}`)
  console.log(
    `  tileCompression:     ${header.tileCompression === 2 ? 'gzip' : header.tileCompression}`,
  )
  console.log()
  ...
  console.log(
    `Recommended demo URL: #${Math.max(minVisibleZoom, 12)}/${header.centerLat}/${header.centerLon}`,
  )
}
