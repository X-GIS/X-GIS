...
describe('forecast-hour step cost (#1367)', () => {
  it('THE FIX: a data swap re-derives the COVERAGE arm, not the whole scene', () => {
    ...
    expect(setCoverageData()).toContain('_armCoverageFields(handle)')
    expect(setCoverageData(), 'a data swap must not rebuild the scene').not.toContain(
      'this.rebuildLayers()',
    )
  })

  it('...and so does a time step', () => {
    expect(setCoverageTime()).toContain('_armCoverageFields(handle)')
    expect(setCoverageTime(), 'a time step must not rebuild the scene').not.toContain(
      'this.rebuildLayers()',
    )
  })
