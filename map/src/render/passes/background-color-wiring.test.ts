...
// + host (no GPU needed — since the F3b port, execute calls
// `ctx.rhiEncoder.beginRenderPass(...)` through requireRhiFrame then
// `pass.end()`), intercepting beginRenderPass to read the RGBA clear TUPLE
// actually requested. On a flat (mercator, projType 0) frame the style
// background-color fills the whole viewport, so clearValue must equal
// `_backgroundColor` exactly.
//
...
type CapturedClear = readonly [number, number, number, number]

/** Drive the real backgroundPass.execute with a fake flat-frame context and
 *  the given host background-color, capturing the clearValue tuple the pass
 *  asks the RHI frame encoder's `beginRenderPass` for (F3b seam). */
function capturedClearValue(
  ...
  } as unknown as FrameContext['rhiEncoder']

  const ctx = {
    rhiEncoder,
    // requireRhiFrame demands the full RHI bridge set; the pass binds only
    // colorView, the others are presence-checked.
...
describe('background-color → clear-value wiring (GPU-free)', () => {
  it('flat (mercator) clear equals the host background-color exactly', () => {
    ...
    expect(c[0]).toBeCloseTo(BG[0], 6)
    expect(c[1]).toBeCloseTo(BG[1], 6)
    expect(c[2]).toBeCloseTo(BG[2], 6)
    expect(c[3]).toBeCloseTo(BG[3], 6)
  })
  ...
  it('the clear tracks a DIFFERENT background-color (non-vacuous)', () => {
    ...
    expect(c[0]).toBeCloseTo(other[0], 6)
    expect(c[1]).toBeCloseTo(other[1], 6)
    expect(c[2]).toBeCloseTo(other[2], 6)
    expect(c[3]).toBeCloseTo(other[3], 6)
  })
})
