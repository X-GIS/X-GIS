...
function harness(opts: { rawShell?: boolean; noAccum?: boolean } = {}) {
  ...
  const ctx = {
    ...
    // `rhi` is a REQUIRED FrameContext field that the `as unknown as` cast let
    // this stub omit. The pass itself no longer reads it — the overdraw mode is
    // gated once at the frame level (FrameContext.overdraw) rather than per pass
    // (#1046 Inc-F2d review F2) — but the field belongs on any FrameContext stub.
