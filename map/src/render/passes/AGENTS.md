...
| `overdraw-compose-pass.ts` | Debug-only bucket: reads the `r16float` fragment-count accumulator and colourmap-composites it to the swapchain. Active only when the frame's `scene.overdraw` is set at build time; runs last so it writes the final swapchain attachment.                                                                                                                                                                                                                                                                                                                                                                                        |
...
- **`scene.overdraw` / `ctx.overdraw` — NEVER `DEBUG_OVERDRAW` in this directory.** The URL flag is a build-time constant; the FRAME truth is device-aware (the mode cannot run where the compose is raw WebGPU), and a pass reading the flag while the targets were routed from the frame truth is a half-gated frame (#1046 Inc-F2d). Evaluate at `shouldRun` time via `scene.overdraw`; `execute` has `ctx.overdraw`.
...
- `../frame-context` — `FrameContext`: per-frame GPU resources (encoder, textures, MSAA views) + the opaque `projection` token (engine-blind; content unwraps it via `../projection-token`)
...
- `../../debug-flags` — debug URL flags (`DEBUG_RHI_CHECKER`); overdraw is read from `scene.overdraw`/`ctx.overdraw`, never the raw `DEBUG_OVERDRAW` const
