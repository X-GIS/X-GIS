...
// `render-phases/`, so a gate still reading only the class would stop seeing the
// code it pins — and would go GREEN having stopped, which is the vacuity §12
// keeps paying for. This returns the class plus its phases, restoring exactly the
// scope those assertions were written against; the counting ones ("exactly one
// `this._drapeGlobeFills = false`") in particular are only meaningful over the
// whole path.
//
...
const PHASES = join(HERE, 'render-phases')

/** The phase modules take the renderer as a parameter named `vtr`, where the
 ...
 *
 *  Scoped to the phase modules, and `vtr` is only ever the phase parameter, so
 *  this cannot rewrite an unrelated identifier. */
const asReceiver = (phaseSrc: string): string => phaseSrc.replace(/\bvtr\./g, 'this.')

/** `vector-tile-renderer.ts` followed by every phase module, name-sorted so the
 *  concatenation is stable and a gate's index arithmetic is reproducible. */
export function renderPathSource(): string {
  const phases = readdirSync(PHASES)
    .filter((f) => f.endsWith('.ts'))
    .sort()
  return [
    readFileSync(join(HERE, 'vector-tile-renderer.ts'), 'utf8'),
    ...phases.map((f) => asReceiver(readFileSync(join(PHASES, f), 'utf8'))),
  ].join('\n')
}
