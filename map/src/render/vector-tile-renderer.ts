...
import type { GPUTile, LayerDrawPhase, RenderArgs, LayerSlot } from './vector-tile-renderer-types'
...
export class VectorTileRenderer {
  ...
  private renderTileKeys(
    ...
    for (let ki = 0; ki < keys.length; ki++) {
      ...
      let slotOffset = 0
      let currentTileBg: GPUBindGroup | null = this._bindGroups.baseGroup()
      if (!skipPack) {
        ...
        slotOffset = this.allocUniformSlot()
