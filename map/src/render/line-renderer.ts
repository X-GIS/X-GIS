...
export class LineRenderer {
   ...
   *  caller's pool + encoder). Returns the destination buffer + the
   *  borrowed staging slot (null = nothing to release) — the caller
   *  submits the encoder, then hands the slot back via `pool.release`
   *  (#784: the slot rides through instead of a per-upload closure).
