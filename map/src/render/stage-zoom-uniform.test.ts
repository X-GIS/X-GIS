...
`
  ...
  return variant as ShaderVariantInfo
}
...
describe('the `zoom` builtin resolves in every composer host (#1635)', () => {
  ...
  it('point: EACH axis carries the read on its own, not just the pair', () => {
    // Point's headline property is that `@color` and `@stroke` compose
    // INDEPENDENTLY, and a half-wired seam is silent (the unwired axis falls back
    // to the CPU-baked default read). Counting `u.zoom` occurrences in the
    // both-axes shader would NOT prove it — the emitter CSEs the shared
    // `u.zoom / 22.0` into one temp both axes then reference. Composing each axis
    // ALONE removes the shared subexpression entirely: if only the fill axis were
    // wired, the stroke-only shader would carry no `u.zoom` at all.
    for (const [axis, body] of [
      ['@color', ZOOM_BODY_COLOR],
      ['@stroke', ZOOM_BODY_STROKE],
    ] as const) {
      const spec = toComposerPointVariant(variantFor(body))
      expect(spec, `${axis}: the compiler variant was rejected by the point bridge`).not.toBeNull()
      expect(emitPointWgsl(spec), `${axis}-only: the axis does not carry the zoom read`).toContain(
        ZOOM_UNIFORM_REF,
      )
    }
  })
  ...
  it('polygon: a @color stage block reads the lane it has carried since the palette work', () => {
    ...
    //
    // The body reads `.r` as well as `zoom`, unlike the line/point fixtures above:
    // `toComposerVariant` returns null for a variant with neither a preamble nor a
    // feature buffer (that slice draws through the default-uniform path and never
    // reaches the composer), so a zoom-ONLY body would assert nothing here. The
    // field read is what routes this through the composer; `zoom` still takes no
    // storage slot beside it (compiler-side zoom-uniform.test.ts pins that).
    const spec = toComposerVariant(variantFor('@color { return vec4(zoom / 22, .r, 0.1, 1) }'))
