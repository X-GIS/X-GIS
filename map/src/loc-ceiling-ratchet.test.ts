...
const CEILINGS: Record<string, number> = {
  ...
  // 5471→5475 (#2118, on top of #2116's 5462→5471). #2116 added the glyph keep-alive in
  // `shouldRenderThisFrame` (+9, and it sits here rather than behind a forwarder because
  // this method is the single authority gating both rendering and `idle`). #2118 adds 4:
  // the ONE line only map.ts can write — `show.circlePitchAlignmentMap` into the
  // geojson-point `addLayer` tail — plus its three-line note on why the knob is trailing.
  // Composition-root wiring, nothing extract-worthy (§2). MEASURED post-merge (`wc -l`),
  // set EXACTLY to the count — headroom is re-justified per phase, never banked. Lower as
  // #991 decomposes map.ts.
  'map/src/map.ts': 5475,
  ...
  'compiler/src/ir/emit-commands.ts': 812,
