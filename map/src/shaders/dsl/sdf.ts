...
export const dist_to_quadratic = fn(
  'dist_to_quadratic',
...
export const winding_line = fn(
  ...
  ({ p, a, b }) => {
    ...
    const cross = b.x
      .sub(a.x)
      .mul(p.y.sub(a.y))
      .sub(p.x.sub(a.x).mul(b.y.sub(a.y)))
...
const sdf_shape = fn(
  ...
  ({ uv_in, shape_id }) => {
    ...
    If(
      uv.x
        .lt(s.bbox_min_x)
        .or(uv.x.gt(s.bbox_max_x))
        .or(uv.y.lt(s.bbox_min_y))
        .or(uv.y.gt(s.bbox_max_y)),
      () => {
        Return(f32(2))
      },
    )
    ...
    Loop(
      ...
      (i) => {
        ...
        Switch(seg.kind)
          .case(0, () => {
            min_dist.assign(min(min_dist, dist_to_segment({ p: uv, a: seg.p0, b: seg.p1 })))
            winding.assign(winding.add(winding_line({ p: uv, a: seg.p0, b: seg.p1 })))
          })
          .case(1, () => {
            min_dist.assign(
              min(min_dist, dist_to_quadratic({ p: uv, a: seg.p0, b: seg.p1, c: seg.p2 })),
            )
            winding.assign(winding.add(winding_line({ p: uv, a: seg.p0, b: seg.p2 })))
          })
          .case(2, () => {
            min_dist.assign(
              min(min_dist, dist_to_cubic({ p: uv, a: seg.p0, b: seg.p1, c: seg.p2, d: seg.p3 })),
            )
            winding.assign(winding.add(winding_line({ p: uv, a: seg.p0, b: seg.p3 })))
          })
          .default(() => {
            /* default: {} */
          })
