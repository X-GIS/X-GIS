...
function distToQuadRef(p: V2, a: V2, b: V2, c: V2): number {
  ...
  for (let i = 0; i <= 16; i++) {
    const t = i / 16
    best = Math.min(best, dist2(p, mix(mix(a, b, t), mix(b, c, t), t)))
  }
  return best
}
...
function windingRef(p: V2, a: V2, b: V2): number {
  if (a[1] <= p[1]) {
    if (b[1] > p[1]) {
      const cv = (b[0] - a[0]) * (p[1] - a[1]) - (p[0] - a[0]) * (b[1] - a[1])
      if (cv > 0) return 1
    }
  } else {
    if (b[1] <= p[1]) {
...
const seg = (kind: number, p0: V2, p1: V2, p2: V2 = [0, 0], p3: V2 = [0, 0]): Seg => ({
  kind,
  color_idx: 0,
  flags: 0,
  _pad: 0,
...
function sdfRef(uvIn: V2, shapes: Shape[], segs: Seg[], shapeId: number): number {
  ...
  if (uv[0] < s.bbox_min_x || uv[0] > s.bbox_max_x || uv[1] < s.bbox_min_y || uv[1] > s.bbox_max_y)
    return 2
  let md = 1e10,
    w = 0
  ...
  for (let i = s.seg_start; i < end; i++) {
    ...
    switch (g.kind) {
      case 0:
        md = Math.min(md, distToSegmentRef(uv, g.p0, g.p1))
        w += windingRef(uv, g.p0, g.p1)
        break
      case 1:
        md = Math.min(md, distToQuadRef(uv, g.p0, g.p1, g.p2))
        w += windingRef(uv, g.p0, g.p2)
        break
      case 2:
        md = Math.min(md, distToCubicRef(uv, g.p0, g.p1, g.p2, g.p3))
        w += windingRef(uv, g.p0, g.p3)
        break
    }
  }
...
describe('PoC-B: SDF dist/winding helpers (cpu interpreter ↔ hand reference)', () => {
  ...
  it('dist_to_quadratic / dist_to_cubic loops match reference (16/24-step)', () => {
    const p: V2 = [0.3, 0.2]
    expect(M.fns.dist_to_quadratic(p, [-1, 0], [0, 1], [1, 0]) as number).toBeCloseTo(
      distToQuadRef(p, [-1, 0], [0, 1], [1, 0]),
      6,
    )
    expect(M.fns.dist_to_cubic(p, [-1, 0], [-0.3, 1], [0.3, -1], [1, 0]) as number).toBeCloseTo(
      distToCubicRef(p, [-1, 0], [-0.3, 1], [0.3, -1], [1, 0]),
      6,
    )
  })
})
...
describe('PoC-B: WGSL backend emits a structurally valid sdf shader', () => {
  ...
  it('declares the storage structs + bindings', () => {
    ...
    expect(wgsl).toContain(
      '@group(0) @binding(9) var<storage, read> segments: array<ShapeSegment>;',
    )
  })
