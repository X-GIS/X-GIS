...
| interpolate (cubic-bezier) | low | Numeric-valued zoom interpolates densify at compile time into a piecewise-linear approximation (6 samples per segment, CSS bezier-eased via Newton-Raphson). Runtime sees a longer linear stop list and visually approximates the bezier curve. Non-numeric values (colour stops) still warn and fold to pure linear. Iter 60 landing. |
| interpolate-hcl | low | Zoom interpolates over hex colour stops densify in LCh (polar Lab) space at compile time (iter 61, 6 samples per segment, hue shortest-path). Runtime sees a longer linear-sRGB hex stop list and visually approximates the LCh-space interpolation. Non-zoom inputs + non-hex stops fall back to linear-sRGB with a graceful-downgrade warning. |
| interpolate-lab | low | Zoom interpolates over hex colour stops densify in Lab (D50) space at compile time (iter 61, 6 samples per segment). Runtime sees a longer linear-sRGB hex stop list and visually approximates the Lab-space interpolation. Non-zoom inputs + non-hex stops fall back to linear-sRGB with a graceful-downgrade warning. |
