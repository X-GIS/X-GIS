...
- To regenerate the fixture after an intentional formula change:
  ...
  ```
  Then commit the updated `data/src/cross-validation.fixture.json`.
...
- After regenerating: run `bun run test` (or `bun scripts/precheck.ts`) to confirm `data/src/cross-validation.test.ts` passes against the new fixture.
...
- All projections use `R = 6378137` (matching `EARTH_RADIUS` in `geo/src/projection.ts`).
...
- Writes: `data/src/cross-validation.fixture.json`
- Read by: `data/src/cross-validation.test.ts`
