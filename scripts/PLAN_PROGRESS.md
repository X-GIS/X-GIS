...
| Spec-strict + warning cleanup sweep (iter 65-95) | 31-iter sweep tightening compiler-side spec-strict validation + promoting deferred gaps from generic ignoredText to specific layer-level warnings. Each gate-class adds named diagnostics for typos/spec violations that previously fell through silently. **Spec-strict gates added**: monotonicity (interpolate + step stops, iter 74-75), duplicate labels (match, iter 76), duplicate let-bindings (iter 77), unknown interpolate curve type (iter 78), source scheme unknown value (iter 79), text-font not-array (iter 80), icon-rotation-alignment enum (iter 81), cubic-bezier CP count (iter 82), exponential base ≤ 0 (iter 83). **Negative-clamp warning sweep (iter 84-95)**: 12 paint-numeric properties (text-size, line-width, circle-radius, fill-extrusion-height/base, text-padding, text-line-height, text-halo-width/blur, circle-stroke-width, line-blur, opacity range) now surface the silent clamp-to-zero. **Specific gap-warning promotions (iter 88-91)**: 8 Plan §4 deferred properties (icon-color, icon-halo-color/-width/-blur, icon-text-fit, text-writing-mode, text-max-angle, symbol-z-order, symbol-avoid-edges) lifted out of generic ignoredText aggregator into specific layer-level warnings. **Integration gate (iter 67)**: densified-bezier end-to-end through convert → lex → parse → lower → evaluate. **Densification follow-ups (iter 71-72)**: v8-strict literal-wrap unwrap for cubic-bezier CPs; drift gate orphan ceiling 10 → 3. |
