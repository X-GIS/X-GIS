...
## 3. Concepts  `/docs/concepts/*`  *(EXPAND 4 → 11)*
...
- Existing: Pipeline · Compute paint · Projections · RTC precision *(restyle to kit)*.
...
## 8. Blog  `/blog`  *(shipped — grow content)*
...
## 11. API  `/docs/api`  *(restyle + complete)*
...
Real direction, vision-framed (no dates): **Google-Earth-class 3D globe** (terrain · 3D-tiles · streaming · LOD · fly-to) · **Custom layers** (three.js/Babylon/raw-WebGPU interop — *not built; the engine has the seam*) · **Full Mapbox parity** (175→235) · **Embeddable engine** (@xgis/engine + @xgis/map split, in progress).

---

## Build priority
1. **P2** — Globe concept ⭐ + Roadmap band + Why page. *(identity + convince)*
2. **P3** — Playground (try-it) + Examples/Shaders/Convert restyle. *(the wow + consistency)*
