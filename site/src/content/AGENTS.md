...
| File                    | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `gallery-demos.ts`      | Exports `Demo` interface, `Category` interface, `galleryCategories: Category[]` (10 categories, ~45 demos — one card per capability, see the curation rule in the file header), `unlistedDemoIds: string[]` (playground demos deliberately not carded), `featuredDemos: Demo[]` (3 entries), and `runIdOf(d)` helper. Each `Demo` carries `id`, optional `runId`, `title`, `body`, and optional `noThumb`, `defaultHash`, `devOnly`, `standaloneUrl`. Thumbnail filename convention: `public/thumbnails/{id}.jpg`. |
...
- `gallery-demos.ts` is the ONLY place to add, remove, or rename gallery demos. Do not add demo metadata directly in `examples.astro`. A new playground demo must land EITHER as a card or in `unlistedDemoIds` — the build-time gallery-drift gate (`lib/gallery-drift-check.ts`) fails the build otherwise.
