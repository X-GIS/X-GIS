...
safest move imaginable: `git mv` it to the neutral engine and go. It *was* clean.
The catch, which took proving to trust, is that it was the *only* class in map's
...
appearance. You have to *prove* each one.

So the method that worked was not "start editing." It was "sort first, then
prove each move." Every one of those 700 references is a *different kind* of
...
*used* as a backend object. The counterintuitive part: most of map's
...
"pipeline" only to match it *by stable factory label* to a pre-built RHI
...
*creates* a native `GPURenderPipeline` and it flows structurally into the
...
onto the first draw, exactly the kind of change that *should* be inert — moved
that 16-byte write to *after* `addLayer`. A data-path test that had been
...
the capture by buffer *label* rather than by write order — a correctness
...
*behavioural equivalence of a translation* rather than *no-op of a retype*. The
...
must share the render loop's *one* command encoder and *one* submit, or the DC=0
...
name no GPU handle; and among the *classes with a runtime body*, `FrameArena`
