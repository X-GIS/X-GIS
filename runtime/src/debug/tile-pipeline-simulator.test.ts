...
import { simulateTilePipeline, makePitchSweep } from './tile-pipeline-simulator'
...
describe('tile pipeline simulator — basic behavior', () => {
  ...
  it('upload budget throttles the convergence rate', () => {
    ...
    const fastResult = simulateTilePipeline(
      trajectory,
      { maxLevel: 7 },
      {
    ...
    const slowResult = simulateTilePipeline(
      trajectory,
      { maxLevel: 7 },
      {
    ...
    expect(fastResult.perFrame[3].missedCount).toBeLessThan(slowResult.perFrame[3].missedCount)
  })
})
...
describe('tile pipeline simulator — FLICKER reproduction', () => {
  ...
  it('a larger upload budget reduces peak missed count for the same trajectory', () => {
    ...
    const stockBudget = simulateTilePipeline(
      traj,
      { maxLevel: 10 },
      {
    ...
    const fatBudget = simulateTilePipeline(
      traj,
      { maxLevel: 10 },
      {
