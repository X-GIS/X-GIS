...
describe('runCompile — point features', () => {
  it('produces one tile at z0 with pointVertices matching input count', () => {
    ...
    // ECEF-DSFUN stride-13: (ex_h, ey_h, ez_h, ex_l, ey_l, ez_l, feat_id, abs_lon,
    // abs_lat, mx_h, mx_l, my_h, my_l) × 3 points = 39 floats.
    expect(tile.pointVertices!.byteLength).toBe(3 * 13 * 4)
  ...
  it('idResolverMode=feature-id-fallback encodes feature.id into feat_id slot', () => {
    ...
    // ECEF-DSFUN stride-13; index 6 = feat_id. Point 0 feat_id at 6, point 1 at 13+6=19.
    expect(pv[6]).toBe(123)
    expect(pv[19]).toBe(456)
  })
})
...
describe('runCompile — polygon features', () => {
  it('tessellates a square into two triangles (6 indices)', () => {
    ...
    // #398 quantized polygon vertex layout: stride 28 bytes = 7 floats
    // `[uint16×6 position | fid | abs_lon | abs_lat | true_lat]`.
    // 4 unique corners = 28 floats. Dequant params travel on the slice.
    expect(verts.length).toBe(4 * 7)
