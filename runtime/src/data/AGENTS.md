...
| File | Description |
|------|-------------|
| `tile-catalog.ts` | `TileCatalog` — routes requests to `TileSource` backends, manages dataCache (byte + count LRU eviction), concurrency budget, sub-tile generation, synthesised `XGVTIndex`, per-backend sink closures that stamp `TileData.originBackend`, and `getScheme()` accessor. Primary surface the VTR depends on. (62 KB; formerly `XGVTSource`.) |
