...
describe('GeoJSONPolarCapBackend packed output', () => {
  it('a pole vertex reaches the TRUE north pole ECEF via the quantized lanes (NOT the f32 tail)', () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const ex =
        (u16[u]! * 65536 + u16[u + 1]!) * res.dequantScale - res.dequantHalf + Z0_TILE_CENTER[0]
      const ey =
        (u16[u + 2]! * 65536 + u16[u + 3]!) * res.dequantScale - res.dequantHalf + Z0_TILE_CENTER[1]
      const ez =
        (u16[u + 4]! * 65536 + u16[u + 5]!) * res.dequantScale - res.dequantHalf + Z0_TILE_CENTER[2]
    ...
    let sawPole = false
    for (let i = 0; i < count; i++) {
      if (Math.abs(verts[i * FILL_FLOATS_PER_VERT + FILL_TRUELAT_FLOAT]! - 90) < 1e-2)
        sawPole = true
    }
