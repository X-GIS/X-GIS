...
const vs = entryFn('vs_heatmap', 'vertex', [
  ...
  const radiusPx = b.let('radius_px', max(featData.at(fid.mul(STRIDE).add(u32(0)), f32T), f32(1)))
  const weight = b.let('weight', featData.at(fid.mul(STRIDE).add(u32(1)), f32T))
  ...
  const offXY = b.let('off_xy', offsets.at(p.quad_id, vec2fT))
  const pxToNdc = b.let('px_to_ndc', vec2(f32(2).div(viewport.x), f32(2).div(viewport.y)))
  const offsetPx = b.let('offset_px', vec2(offXY.x.mul(radiusPx), offXY.y.mul(radiusPx)))
  const offsetNdc = b.let('offset_ndc', offsetPx.mul(pxToNdc))
  const quadClip = b.let('quad_clip', centerClip.add(vec4(offsetNdc.mul(centerClip.w), f32(0), f32(0))))

  const out = b.var('out', structT('HeatOut'))
  b.assign(out.field('position', vec4fT), quadClip)
  b.assign(out.field('uv', vec2fT), offXY)
  b.assign(out.field('weight', f32T), weight)
  b.ret(out)
})
...
const fs = entryFn('fs_heatmap', 'fragment', [{ name: 'in', type: structT('HeatOut') }], vec4fT, (b, p) => {
  const uv = b.let('uv', p.in.field('uv', vec2fT))
  const d2 = b.let('d2', uv.x.mul(uv.x).add(uv.y.mul(uv.y)))
  const ZERO = f32(0.22313016) // exp(-1.5)
  const gauss = b.let('gauss', max(exp(d2.mul(-1.5)).sub(ZERO), f32(0)))
  const density = b.let('density', gauss.mul(p.in.field('weight', f32T)))
  // R16Float target — only .r carries density; gba unused.
  b.ret(vec4(density, f32(0), f32(0), f32(1)))
}, '@location(0)')
