...

    {
      ...
      for (let gi = 0; gi < opaqueCount; gi++) {
        ...
        ctx.passScope(isFirst ? 'opaque-main' : `opaque[${gi}]`, () => {
          ...
          const tsWrites = this.host.gpuTimer?.passWrites() || undefined
          ...
          if (isFirst) {
            ...
            // interaction.
            this.host.backgroundRenderer?.render(subPass)
            this.host.gpuTimer?.mark(subPass, 'after_bg')
            ...
            // renderer's uniform.
            if (this.host._rasterShow) {
              const op = resolveNumberShape(
                this.host._rasterShow.paintShapes.opacity,
                this.host.camera.zoom, this.host._elapsedMs,
              ).value
              this.host.rasterRenderer.setOpacity(op)
            } else {
          ...
          if (group) {
            ...
            const drawShow = (cs: typeof group.shows[number]) => {
              const debugFp = DEBUG_OVERDRAW
                ? (cs.bgl === this.host.renderer.featureBindGroupLayout
                    ? this.host.renderer.fillPipelineOverdrawFeature!
                    : this.host.renderer.fillPipelineOverdraw!)
                : null
              const debugLp = DEBUG_OVERDRAW ? this.host.renderer.linePipelineOverdraw! : null
