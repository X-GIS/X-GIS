...
export class VectorTileRenderer {
  ...
  // Faithful drape bake (US-002): when capturing is on, each layer's resolved
  // fill RGBA is recorded (keyed by its gpuCache inner map) during the normal
  // render so the bake can paint real per-layer colours instead of one green.
  captureBakeColors = false
  private _bakeLayerColors = new Map<Map<number, GPUTile>, number[]>()
  ...
  // ── PoC US-S2 (bake): render one cached tile's flat fill to an
  //    offscreen rgba8unorm texture for the globe texture-drape PoC.
  //    EXPERIMENTAL — only reached from the __xgisDebugBakeDrape hook in
  //    opaque-pass; ZERO effect on normal rendering. See .omc/progress.txt.

  /** Return a resident cached tile with drawable polygon geometry
   *  (indexCount > 0), plus its decoded z/x/y. Used by the bake-drape debug
   *  hook to pick a tile to bake. When (preferLon, preferLat) is given, picks
   *  the non-empty tile whose geographic centre is nearest that point — so the
   *  baked patch lands on the FRONT hemisphere of a globe view (an arbitrary
   *  first tile may be back-facing and hidden). Else returns the first one.
   *  Null when no tile has fill geometry yet. */
  firstNonEmptyCachedTile(
    preferLon?: number,
    preferLat?: number,
  ): { cached: GPUTile; z: number; x: number; y: number } | null {
    let first: { cached: GPUTile; z: number; x: number; y: number } | null = null
    let best: { cached: GPUTile; z: number; x: number; y: number } | null = null
    let bestDist = Infinity
    const havePrefer = preferLon !== undefined && preferLat !== undefined
    for (const layerCache of this.gpuCache.values()) {
      for (const [key, cached] of layerCache) {
        if (cached.indexCount <= 0) continue
        const [z, x, y] = tileKeyUnpack(key)
        if (!first) first = { cached, z, x, y }
        if (!havePrefer) continue
        const n = Math.pow(2, z)
        const cLon = ((x + 0.5) / n) * 360 - 180
        const cLat =
          (Math.atan(Math.sinh(Math.PI * (1 - 2 * (y + 0.5) / n))) * 180) / Math.PI
        // Angular distance to the prefer point (degrees, simple L2 in lon/lat —
        // good enough to choose a front-facing tile, no great-circle needed).
        const dLon = cLon - (preferLon as number)
        const dLat = cLat - (preferLat as number)
  ...
  /** PoC US-S2 — render ONE cached tile's flat fill into an offscreen
   *  rgba8unorm texture (sizePx × sizePx), as a solid recognizable colour so
   *  the bake's vector content is visible. Reuses the depth-disabled ground
   *  fill pipeline (`fillGround`, vs_main_ecef flat path) + the base bind-group
   *  resources (palette/sprite stubs), swapping only the uniform at binding 0
   *  for a from-scratch 256-byte bake uniform that maps the tile's local
   *  Mercator [0,extent] → NDC[-1,1] via an ORTHO matrix (coordinate math in
   *  .omc/progress.txt). Returns the offscreen texture (TEXTURE_BINDING |
   *  COPY_SRC) for the drape. Returns null if the ground pipeline / base layout
   *  / palette resources are not wired yet. */
  /** All resident non-empty (indexCount>0) layer tiles at a given z/x/y, across
   *  every per-layer cache. Lets the bake COMPOSITE all of a tile's layers
   *  (water+landuse+roads+buildings) into one texture so it shows the full map
   *  content, not one arbitrarily-picked sparse layer. */
  cachedLayersAt(z: number, x: number, y: number): GPUTile[] {
    const out: GPUTile[] = []
    for (const layerCache of this.gpuCache.values()) {
      for (const [key, t] of layerCache) {
        if (t.indexCount <= 0) continue
        const [tz, tx, ty] = tileKeyUnpack(key)
        if (tz === z && tx === x && ty === y) out.push(t)
      }
    }
    return out
  }

  /** Monotone-ish generation of captured bake colours — grows as layers are
   *  first seen this run. The bake cache key folds this in so a tile baked
   *  before its colours were captured re-bakes once they are. */
  get bakeColorGen(): number { return this._bakeLayerColors.size }

  /** Faithful-bake variant of cachedLayersAt: each layer tile paired with its
   *  resolved fill RGBA (captured during normal render via `captureBakeColors`).
   *  Falls back to a neutral green for any layer not yet captured. Iteration is
   *  gpuCache order (z-asc-ish per insertion) so the bake composites bottom-up. */
  cachedLayersWithColorAt(z: number, x: number, y: number): Array<{ tile: GPUTile; color: number[] }> {
    const out: Array<{ tile: GPUTile; color: number[] }> = []
    for (const layerCache of this.gpuCache.values()) {
      const color = this._bakeLayerColors.get(layerCache) ?? [0, 0.8, 0.2, 1]
      for (const [key, t] of layerCache) {
        if (t.indexCount <= 0) continue
        const [tz, tx, ty] = tileKeyUnpack(key)
        if (tz === z && tx === x && ty === y) out.push({ tile: t, color })
      }
    }
    return out
  }

  /** Distinct resident tile coords (z/x/y) that have drawable polygon geometry,
   *  sorted by zoom ascending (low-z first = painter's order: parents drawn
   *  before children). Used by the multi-tile drape PoC to bake+drape every
   *  resident tile of the scene on the globe, not just one debug tile. */
  residentTileKeys(): Array<{ z: number; x: number; y: number }> {
    const seen = new Set<number>()
    const out: Array<{ z: number; x: number; y: number }> = []
    for (const layerCache of this.gpuCache.values()) {
      for (const [key, t] of layerCache) {
        if (t.indexCount <= 0 || seen.has(key)) continue
        seen.add(key)
        const [z, x, y] = tileKeyUnpack(key)
        out.push({ z, x, y })
      }
  ...
  bakeTileToTexture(tiles: GPUTile[], tileZoom: number, sizePx: number, colors?: number[][]): GPUTexture | null {
    if (tiles.length === 0) return null
    const device = this.device
    const pipeline = this._bindGroups.groundPipeline()
    const baseLayout = this._bindGroups.baseLayout()
    const pal = this._bindGroups.paletteResources()
    if (!pipeline || !baseLayout || !pal.paletteColorAtlasView || !pal.paletteSampler || !pal.spriteAtlasView) {
      return null
    }
    // The bake builds a single colour attachment. When picking is enabled the
    // ground pipeline has a SECOND (rg32uint) colour target → attachment-count
    // mismatch. Default quality is picking:false; bail out cleanly otherwise.
    if (isPickEnabled()) {
      xlog.warn('[bake-drape] skipped — picking enabled (single-attachment bake unsupported)')
      return null
    }

    const sampleCount = getSampleCount()
    // The fillGround pipeline's colour target is the swapchain format
    // (`this.format`, typically bgra8unorm). The render-pass colour
    // attachment MUST match it, so the offscreen is that format — NOT a
    // hardcoded rgba8unorm (which would fail format-match validation). The
    // drape samples via `texture_2d<f32>` so bgra storage still yields the
    // logical RGBA colour; channel order is handled at sample time.
    const offFormat = this.format

    // Offscreen single-sample target the drape will sample (+ copy src).
    const offscreen = device.createTexture({
      label: 'bake-drape-offscreen',
      size: { width: sizePx, height: sizePx },
      format: offFormat,
      usage: GPUTextureUsage.RENDER_ATTACHMENT | GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_SRC,
      sampleCount: 1,
    })
    // MSAA color target (resolved into `offscreen`) when MSAA is on. The
    // fillGround pipeline bakes `multisample.count = getSampleCount()`, so the
    // pass attachments MUST match that sample count.
    const msColor = sampleCount > 1
      ? device.createTexture({
          label: 'bake-drape-ms-color',
          size: { width: sizePx, height: sizePx },
          format: offFormat,
      ...
      : null
    // Depth-stencil for the pass (fillGround depthStencil = depth24plus-stencil8).
    const depthTex = device.createTexture({
      label: 'bake-drape-depth',
      size: { width: sizePx, height: sizePx },
    ...
    // ── Bake uniform (256 bytes = Float32Array(64)). Float offsets per the
    //    polygon Uniforms struct (polygon.ts:48-102 / progress.txt).
    const EARTH_R = 6378137
    const e = (2 * Math.PI * EARTH_R) / Math.pow(2, tileZoom) // tile extent (metres)
    // ONE 256B-aligned uniform slot PER layer tile so each draws with its OWN
    // resolved fill colour (faithful bake) — falls back to neutral green for any
    // layer whose colour wasn't captured. Shared ortho mvp; only fill_color
    // varies. Slot layout mirrors the polygon Uniforms struct.
    const SLOT_F32 = 64, SLOT_B = 256
    const N = tiles.length
    const slotData = new Float32Array(N * SLOT_F32)
    for (let i = 0; i < N; i++) {
      const o = i * SLOT_F32
      // mvp[0-15] — column-major ORTHO localMerc [0,e]² → NDC[-1,1]².
      slotData[o + 0] = 2 / e; slotData[o + 5] = 2 / e; slotData[o + 10] = 1
      slotData[o + 12] = -1; slotData[o + 13] = -1; slotData[o + 15] = 1
      // fill_color[16-19] — this layer's resolved RGBA (or neutral green).
      const c = colors?.[i] ?? [0, 0.8, 0.2, 1]
      slotData[o + 16] = c[0]; slotData[o + 17] = c[1]; slotData[o + 18] = c[2]; slotData[o + 19] = c[3]
      slotData[o + 24] = 0   // proj_params.x < 0.5 → flat Mercator branch
      slotData[o + 34] = 1   // opacity
      slotData[o + 40] = -1e9; slotData[o + 41] = -1e9; slotData[o + 42] = 1e9; slotData[o + 43] = 1e9 // clip_bounds (none)
    }
    const bakeBuf = device.createBuffer({
      label: 'bake-drape-uniform',
      size: Math.max(256, N * SLOT_B),
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    })
    device.queue.writeBuffer(bakeBuf, 0, slotData)

    // Bake bind group — base layout (bindings 0/2/4/5/6); binding 0 = bake
    // uniform, 2/4/5/6 = reuse the live palette/sprite resources (matches the
    // base `tileBgDefault` group). Binding 0 of the base layout is dynamic-
    // offset, so the draw binds it with offset [0].
    const bakeBg = device.createBindGroup({
      label: 'bake-drape-bg',
      layout: baseLayout,
      entries: [
        { binding: 0, resource: { buffer: bakeBuf, offset: 0, size: 256 } },
        { binding: 2, resource: pal.paletteColorAtlasView },
        { binding: 4, resource: pal.paletteSampler },
        { binding: 5, resource: pal.spriteAtlasView },
        { binding: 6, resource: pal.paletteSampler },
      ],
    })

    const encoder = device.createCommandEncoder({ label: 'bake-drape-encoder' })
    const colorAttachment: GPURenderPassColorAttachment = msColor
      ? {
          view: msColor.createView(),
          resolveTarget: offscreen.createView(),
    ...
    // Composite EVERY layer's polygons at this z/x/y, each with its OWN slot
    // (per-layer fill colour) so the bake shows the real styled map content
    // (water/landuse/buildings in their colours), not one green silhouette.
    for (let i = 0; i < N; i++) {
      const t = tiles[i]
      if (t.indexCount <= 0) continue
      pass.setBindGroup(0, bakeBg, [i * SLOT_B])
      pass.setVertexBuffer(0, t.vertexBuffer, t.polyVertexOffset, t.polyVertexByteLength)
      pass.setIndexBuffer(t.indexBuffer, 'uint32', t.polyIndexOffset, t.polyIndexByteLength)
      pass.drawIndexed(t.indexCount)
    }
    pass.end()
    device.queue.submit([encoder.finish()])

    // Transient pass attachments — the offscreen colour survives (returned).
    msColor?.destroy()
    depthTex.destroy()
    return offscreen
  }
  ...
  render(
    ...
    // Faithful-bake capture: record this layer's resolved fill RGBA so the drape
    // bake can paint it (keyed by the layer's gpuCache map; read-only side table).
    if (this.captureBakeColors && this.cachedFillColor[3] > 0.004) {
      this._bakeLayerColors.set(layerCache, [this.cachedFillColor[0], this.cachedFillColor[1], this.cachedFillColor[2], this.cachedFillColor[3]])
    }
    // iter-185 — line-pattern Stage 2 packs the sprite atlas UV bbox
    // into the stroke_color slot. fs_line_pattern reads
