...
// Construct a VTR instance bypassing GPU init.
function makeVtr(catalog: unknown, neededKeys: number[], frameId: number): VectorTileRenderer {
  ...
  ;(vtr as unknown as { _prevPanCam: unknown })._prevPanCam = null
...
describe('VectorTileRenderer.pumpPrefetch — frame-scope orchestration', () => {
  ...
  it('captures _prevPanCam so the second frame can project velocity', () => {
    ...
    vtr.pumpPrefetch(camera, 0, 1024, 768, 1)
    const prev = (vtr as unknown as { _prevPanCam: { cx: number; cy: number; zoom: number; t: number } | null })._prevPanCam
