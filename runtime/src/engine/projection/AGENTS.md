...
| `camera-helpers.ts` | Pure column-major 4×4 matrix utilities shared by `camera.ts` and `globe.ts`: `invOrthographic` (Snyder azimuthal inverse, limb-clamped), `mulVec4`, `mul4`, `perspectiveMatrix`, `invert4x4`. |
