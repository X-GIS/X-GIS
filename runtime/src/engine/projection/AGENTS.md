...
| `camera-world-copies.ts` | Pure stateless `computeVisibleWorldCopies()` function — the single authority for visible 360°-wrapped copies per frame. Unprojects the nine canvas corners/mid-edge points to z=0, computes the lon range, enumerates integer offsets ±2, and caches by matrix generation ID. Mercator uses NDC-corner unprojection; x-periodic flats (equirect/natural_earth/oblique_mercator) mirror tile-enumeration gates; globe/azimuthal collapse to `[0]`. Replaces prior inlined divergent enumerations (iter-189). Exports `WorldCopiesCache` and `WorldCopiesHost` interfaces for caller state management. |
...
- **CPU↔GPU parity is the central contract.** The WGSL `proj_*` functions in `runtime/src/engine/shaders/projection.ts` are the GPU source of truth; `projection.ts` is the CPU mirror. Any formula change touches both + the DSL graph in `runtime/src/engine/shaders/dsl/projections.ts` (which regenerates the cpu-f64 lowering at `runtime/src/engine/shaders/dsl/cpu-projections.ts`). Mercator clamp, Natural Earth polynomial, and ortho back-face cull have each drifted before — `projection-wgsl-consistency.test.ts` pins parity.
