...
- `QUALITY` is a mutable singleton read at pipeline-creation time. Changes to `msaa` or `picking` require renderer `rebuildForQuality()` calls — never mutate `QUALITY` directly outside `updateQuality`. Read quality via the getter functions `getSampleCount()` / `getMaxDpr()` / `isPickEnabled()` imported from `@xgis/engine` (the single quality authority) — there are no module-load snapshots.
